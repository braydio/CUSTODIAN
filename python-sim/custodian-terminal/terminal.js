(() => {
  ...
  function getCommandIcon(command) {
    ...
    return COMMAND_ICONS[cmd] || COMMAND_ICONS[firstWord] || "▸";
  }
  ...
  function renderCommandIcon(command) {
    ...
    const cmd = (command || "").trim().toUpperCase();
    const maxLen = 36;
    const displayCmd = cmd.length > maxLen ? cmd.slice(0, maxLen - 2) + ".." : cmd;
    return [` ${icon} ${displayCmd}`];
  }
  ...
  async function executeCommandFlow(cmd, options = {}) {
    ...
    if (echo) {
      ...
      state.lastCommand = cmd.toUpperCase();
      appendLines([""]);
      appendLines(renderCommandIcon(cmd));
    }
    try {
