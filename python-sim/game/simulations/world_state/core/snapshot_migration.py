...
def migrate_snapshot(snapshot: dict) -> dict:
    ...
    if version < 7:
        migrated.setdefault(
            "event_context",
            {
                ...
                "ticks_since_assault": 0,
                "ticks_since_hostile": 0,
                "last_assault_tick": None,
            },
        )
        migrated["snapshot_version"] = 7
    ...
    if not isinstance(event_context, dict):
        ...
        event_context.setdefault("recent_events", [])
        event_context.setdefault("last_event_category", None)
        event_context.setdefault("ticks_since_assault", 0)
        event_context.setdefault("ticks_since_hostile", 0)
        event_context.setdefault("last_assault_tick", None)
