...
class GameState:
    ...
    def __init__(self, seed: int | None = None, text_seed: int | None = None):
        ...
        self.last_assault_tick: int | None = None
        self.event_cooldowns = defaultdict(int)
        self.recent_events = deque(maxlen=5)
        self.last_event_category: str | None = None
        self.ticks_since_assault = 0
        self.ticks_since_hostile = 0
    ...
    def snapshot(self) -> dict:
        ...
        return {
            ...
            "event_context": {
                ...
                "last_event_category": self.last_event_category,
                "ticks_since_assault": int(self.ticks_since_assault),
                "ticks_since_hostile": int(self.ticks_since_hostile),
                "last_assault_tick": self.last_assault_tick,
            },
            "sectors": sectors,
    ...
    def from_snapshot(cls, snapshot: dict) -> "GameState":
        ...
        event_context = migrated.get("event_context")
        if isinstance(event_context, dict):
            recent = event_context.get("recent_events", [])
            if isinstance(recent, list):
                state.recent_events.clear()
                for entry in recent[-5:]:
                    state.recent_events.append(str(entry))
            category = event_context.get("last_event_category")
            if isinstance(category, str):
                state.last_event_category = category
            state.ticks_since_assault = int(event_context.get("ticks_since_assault", state.ticks_since_assault))
            state.ticks_since_hostile = int(event_context.get("ticks_since_hostile", state.ticks_since_hostile))
            last_assault_tick = event_context.get("last_assault_tick")
            state.last_assault_tick = int(last_assault_tick) if isinstance(last_assault_tick, int) else None
