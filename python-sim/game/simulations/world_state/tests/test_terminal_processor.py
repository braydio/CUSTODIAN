...
def test_tutorial_relay_topic_is_available() -> None:
    state = GameState()

    result = process_command(state, "TUTORIAL RELAY")

    assert result.ok is True
    assert result.text == "TUTORIAL > RELAY"
    assert result.lines is not None
    assert "SCAN RELAYS (COMMAND) - REVEALS RELAY STATUS AND STABILITY." in result.lines
    assert "SYNC (COMMAND) - CONVERTS PACKETS TO KNOWLEDGE INDEX." in result.lines


def test_tutorial_quickstart_arrn_phases_progress() -> None:
    state = GameState(seed=42)

    process_command(state, "TUTORIAL QUICKSTART")
    process_command(state, "STATUS")
    process_command(state, "POLICY PRESET BALANCED")
    process_command(state, "CONFIG DOCTRINE COMMAND_FIRST")
    process_command(state, "ALLOCATE DEFENSE COMMAND 40")
    process_command(state, "FORTIFY T_NORTH 2")
    process_command(state, "FORTIFY T_SOUTH 2")
    process_command(state, "SCAVENGE 2X")
    process_command(state, "FAB ADD TURRET_AMMO")
    process_command(state, "WAIT 2X")
    process_command(state, "STATUS ASSAULT")

    state.in_major_assault = True
    process_command(state, "STATUS")
    process_command(state, "BOOST DEFENSE COMMAND")
    process_command(state, "REROUTE POWER COMMAND")
    process_command(state, "PRIORITIZE REPAIR COMMAND")
    process_command(state, "WAIT 3X")
    state.in_major_assault = False
    process_command(state, "WAIT 2X")

    result = process_command(state, "STATUS SYSTEMS")
    assert result.lines is not None
    assert any("PHASE 17: RELAY SCAN" in line for line in result.lines)

    result = process_command(state, "SCAN RELAYS")
    assert result.lines is not None
    assert any("PHASE 18: RELAY DEPLOY" in line for line in result.lines)

    result = process_command(state, "DEPLOY NORTH")
    assert result.lines is not None
    assert any("PHASE 18B: RELAY STABILIZE" in line for line in result.lines)
