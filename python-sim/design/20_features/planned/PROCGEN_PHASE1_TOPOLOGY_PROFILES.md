...
Once the ambient event system (AMBIENT_EVENT_DIVERSITY) is implemented, topology profiles can feed event category weights:

| Topology Profile | Event Weight Bias |
|-----------------|-------------------|
| `tunnel_heavy` | +30% HOSTILE (tunnel_infiltration) |
| `open_approach` | +25% RECON (probes, sensor_jam) |
| `fortified_perimeter` | +20% QUIET (stable) |
| `exposed_flanks` | +35% HOSTILE (multi-vector attacks) |

Implementation would add seed-driven modifiers to `compute_category_weights()` in `events.py`:

```python
# In compute_category_weights():
if hasattr(state, 'topology_profile'):
    if state.topology_profile == 'tunnel_heavy':
        weights['HOSTILE'] *= 1.3
    elif state.topology_profile == 'open_approach':
        weights['RECON'] *= 1.25
```

This creates emergent narrative variation: different seeds feel structurally different at the signal level, not just the geometry level.
