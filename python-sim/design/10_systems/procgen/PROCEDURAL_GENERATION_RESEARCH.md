...
Single canonical event: **Power blackout** (maps to `power_blackout` in `game/simulations/world_state/core/events.py`, which reduces power, increases alertness, and applies a decaying `power_drain` effect).
...
- channel: `EVENT`  
- facts: `{ phenomenon: "blackout", system: "power", confirmation: "detected" }`  
- forbid: `{numbers}`
...
[EVENT] LOCAL BLACKOUT CONFIRMED
[STATUS SHIFT] SYSTEM STABILITY DECLINING
```
...
- **Avoid repetition across sessions**: incorporate a “recent phrase memory” per event type so repeated blackouts do not spam identical lines, while still keeping deterministic replay when the same text seed is used.  
