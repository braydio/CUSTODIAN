...
Good correction. The implementation spec needs this assumption changed:

**Confirmed runtime state**

* There is **no inventory system currently**.
* So v1 must add a tiny stack-count ledger, not integrate with an existing inventory manager.
* The shrumb death/drop hook is still a live check item; do **not** duplicate it until you confirm whether it already exists.

I attempted to patch the canvas document directly to reflect that, but the update failed because the exact heading pattern in the canvas didn’t match what the patch expected. So here is the exact block to add/replace in the spec:
...
And Codex should add:

```text
custodian/game/systems/core/systems/inventory_manager.gd
```

with this minimal manager:
