...
- Successful parry now adds a world-space contact spark, an independently owned one-shot success burst, and the positional `content/audio/sfx/combat/parry_success_01.wav` cue at the captured impact point, then attaches a floating BREACH marker plus duration-driven countdown reticle to a critical-open grunt. The sound is committed only inside `_on_parry_success()` and therefore never plays for parry input, expiry, failed-facing checks, or blocked hits. Gameplay timing remains `_parry_critical_window_timer` in `enemy.gd`; VFX/audio only present the confirmed result. Required runtime strips are `content/sprites/effects/combat/critical/combat_fx__parry_success_hit_spark_01__6f__128.png`, `combat_fx__breach_alert__8f__96-48.png`, and `combat_fx__breach_timer_reticle__12f__128.png`. Optional posture-break/expiry strips remain non-blocking and warn once when absent.
