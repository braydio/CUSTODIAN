...
Operator owns input, contextual target selection, reservation request, alignment, the shared direction-specific frame schedule, Operator body/FX playback, the contact-frame damage request, camera impact/hitstop, input lock, and unified cleanup.
...
The south fallback triplet remains eight source frames with the existing nonuniform `1.20s` schedule. The authored east/west triplets contain twelve source frames and advance at a deterministic 12 FPS (`1.00s`). Runtime explicitly assigns the same direction-specific index to Operator body, Operator FX, and the enemy victim. The execution-local `0.11s` contact freeze produces a `1.31s` south presentation or `1.11s` east/west presentation before control restoration.
...
| `critical_execution_victim_e` | `custodian/content/sprites/enemies/enemy_grunt/runtime/body/enemy_grunt__body__melee__critical_execution_victim_01__e__12f__96.png` | 12 × 96×96 / 1152×96 | 12 | no |
| `critical_execution_victim_w` | `custodian/content/sprites/enemies/enemy_grunt/runtime/body/enemy_grunt__body__melee__critical_execution_victim_01__w__12f__96.png` | 12 × 96×96 / 1152×96 | 12 | no |
...
spawn_grunt falcon [x_offset y_offset]
...
The legacy `spawn_grunt x_offset y_offset` form remains valid. `falcon` defaults to a useful launch distance and immediately starts the tracked special windup against the Operator; critical/execution modes default within capture range. `execution_ready` is a semantic alias for an authored hold opportunity; `execution_lethal` uses the same opportunity but reduces the spawned grunt to one health for death-path testing. These modes call an enemy-owned debug setup method only after normal scene instantiation and difficulty setup. They do not add simulation authority to the console, bypass reservation, or create an orphaned `EXECUTING` state.
...
Constraints: preserve the shared zero-offset root, full-cell exports, direction-specific frame schedules, and 110ms contact freeze; do not independently offset layers, mirror the pair, or move damage off source frame 5 (runtime index 4).
