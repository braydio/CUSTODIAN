...
- The Performance page displays recorder state, phase, wall-clock statistics, process/physics split, population/lifetime gauges, top spans, and a deterministic likely-owner classification. Page selection never calls capture or runtime-sampling setters and never requests recursive sampling. A deliberate World/Procgen snapshot request is one-shot; F10 is the normal full-detail path. Performance refresh is capped at 2 Hz, other pages at 4 Hz; unchanged text is not reassigned, counter rows cap at 24, recent events at 10, values at 120 characters, and large dictionaries are summarized for F10 export.
...
- Direct enemy-to-Operator contacts carry a stable attack ID, the authoritative contact-model snapshot, canonical Operator dodge phase/classification, applied damage, health transition, and lethal status. `player_death` retains the lethal attack context even if earlier events leave the bounded buffer. The analyzer correlates these records into lethal and suspicious-hit diagnostics while accepting legacy records with unknown spatial validity.
