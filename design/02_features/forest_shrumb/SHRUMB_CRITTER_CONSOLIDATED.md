# CUSTODIAN — Forest Shrumb Critter: Consolidated Design & Implementation

Status: v1 foundation initialized 2026-04-30  
Purpose: Consolidated lore, gameplay, and runtime implementation reference  
...
## Part I: Core Lore & Gameplay

### The Tragedy of the Forest Shrumb

The Forest Shrumb is a wandering critter that embodies cognitive fragmentation. Once part of a greater consciousness, shrumbs now roam the forest shedding fragments of what they once were.

### Cognitive States

The shrumb's drops affect the player's cognitive state along three axes:

| State | Description |
|-------|-------------|
| `DRIFT` | Scattered thoughts, unfocused |
| `FLOW` | Aligned and coherent |
| `ALIGNMENT` | Purposeful direction |
| `MIXED` | Conflicting cognitive signals |

### Intended Item Trio

| Item ID | Display Name | Rarity | Cognitive Axis | Description |
|---------|--------------|--------|----------------|-------------|
...
## Part II: Runtime Implementation Delta

### 1. Scope

This document defines the runtime delta required to implement the Forest Shrumb design in the current Godot project.

Do not restate the full Forest Shrumb lore design here. Reference Part I above for:
* The Tragedy of the Forest Shrumb
...
### 3. Current Runtime Assumption (Corrected)

There is currently no general inventory system in the project.

Therefore, this feature must include a minimal v1 inventory/resource ledger instead of assuming an existing `InventoryManager` exists.

The v1 inventory scope should be intentionally narrow:

- Track stackable item counts by `item_id`.
- Emit signals when item counts change.
- Provide safe getters for UI and save/load.
- Avoid full RPG inventory UI, equipment grids, drag/drop, sorting, item categories, or stash logic.

The Forest Shrumb drop system should be the first consumer of this inventory ledger, but the ledger should be generic enough to later hold scrap, ammo, med charges, and other stackable resources.

### 4. Required Pre-Implementation Checks
...
### 14. Gameplay Modifier Integration Strategy
...
```text
Implement the Forest Shrumb runtime foundation from design/SHRUMB_CRITTER_CONSOLIDATED.md.

Assume the full Forest Shrumb lore/gameplay design already exists in Part I and should not be restated.
...
- Core lore and gameplay design for the Forest Shrumb critter
- Runtime implementation delta with corrected inventory system assumptions
- All addendum corrections properly integrated into the main content

The document is structured in two parts:
1. **Part I**: Lore, gameplay philosophy, cognitive states, and item definitions
2. **Part II**: Technical implementation details, code examples, and phased rollout strategy

All corrections from the addendum in the original GAMEPLAY_CORE.md have been incorporated directly into the relevant sections (particularly section 3 on runtime assumptions and section 6 on inventory ledger implementation).
