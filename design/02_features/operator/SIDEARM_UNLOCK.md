...
Implementation should use the existing `sidearm_slot_equipped` gate, but default it to false. Add a grant method on Operator, such as `grant_sidearm(...)`, that sets the sidearm slot unlocked/equipped, refreshes weapon frames/visual state, and initializes loaded/reserve ammo if needed.
...
5. pistol profile resolves to 18 damage and 12 magazine size
...
- `grant_sidearm` should unlock/equip the sidearm slot, give the player an initial loaded magazine/reserve ammo, refresh active weapon frames, and update visuals.
...
- Update `operator_ranged_ready_input_smoke.gd`:
  ...
  - assert pistol profile still resolves to 18 damage and 12 magazine size
...
- Operator sidearm fallback now starts locked and is granted through `grant_sidearm(...)`.
