...
The Forlorn Ritualant encounter uses a visual-only shader FX layer to make the broken Ash-Bell chamber read as suspended over a physical cross-continuity intrusion. This is world-space presentation, not gameplay, collision, navigation, or simulation authority.
...
2. `VoidUnderlay/RoomSizedQuad` uses the void-ocean shader and explicit room-mask sampler for slow UV drift/dimming inside a stationary `1280x960` aperture.
...
- Dimensions: `1280x960`, aligned to the mapper-backed perimeter rubble art.
