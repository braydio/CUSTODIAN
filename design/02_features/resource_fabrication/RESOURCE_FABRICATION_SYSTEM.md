...
- Runtime sprite support for generated blackwood deadfall nodes, using harvest-state body frames from `content/sprites/props/harvesting_nodes/blackwood_deadfall/` and FX strips from `content/sprites/effects/harvesting_nodes/blackwood_deadfall/`; each harvest plays the full strike FX, flashes the idle FX frame for the current body state, then advances to the next body state. The separate depleted sheet supplies the sixth/depleted state while the current body strip supplies the five pre-depleted states.
...
**Validation:**
- Walk up to blackwood deadfall → see "CUT BLACKWOOD (0/3)" prompt → press interact 3x → node depletes → blackwood +6 in ledger
...
2. `get_interaction_text()` returns prompt like `"CUT BLACKWOOD (1/3)"`
...
| work_required | 3 | 4 | 2 | 4 |
| yield_amount | 6 | 5 | 8 | 1 |
...
3. InteractionLabel updated with:
   "CUT BLACKWOOD (0/3)"  or  "MINE STRUCTURAL ALLOY (2/4)"  or  "SALVAGE RUIN SCRAP (1/2)"
        │
        ▼
...
Test 1: Basic Harvest
  ...
  2. Walk within 84px -> InteractionLabel shows "CUT BLACKWOOD (0/3)"
  3. Press interact -> label updates to "CUT BLACKWOOD (1/3)"
