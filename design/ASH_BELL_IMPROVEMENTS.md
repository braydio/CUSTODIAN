...
It also fits your existing event structure: the code currently has `has_clapper`, `TAKE_CLAPPER`, `RING_CLAPPER`, `bronze_clapper_pickup`, and `clapper_swing`, so this can replace the clapper without redesigning the whole event.
