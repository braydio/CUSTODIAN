...
| needed | Operator modular lower-body directional idle | `custodian/content/sprites/operator/new_operator/modular/idle/operator__modular_lower_body__idle_01__{n,ne,e,se,s,sw,w,nw}__5f__96.png` | True modular lower-body idle defaults for all directions. | Runtime builder only has a south idle source today; other directions fall back through `action_01` then `run_01`. |
