...
|---|---|---|---|---|
| needed | Sundered Keep labyrinth wall kit | `custodian/content/tiles/sundered_keep/walls/labyrinth_keep/{wall_straight_s_01,wall_pier_01,wall_corner_01,wall_broken_end_01}.png` plus `.game32.json` sidecars | Replace `PLACEHOLDER_sundered_keep_labyrinth_wall_*` sprites with production walls that make courtyard partitions, gatehouse side blockers, and keep maze turns read as actual architecture. | Placeholder runtime assets live under `custodian/content/tiles/sundered_keep/placeholders/walls/` and are intentionally named `PLACEHOLDER_*`. |
| needed | Sundered Keep void/cliff boundary kit | `custodian/content/tiles/sundered_keep/walls/labyrinth_keep/{void_edge_w_01,void_edge_e_01,collapsed_sea_cut_01,cliff_guard_rail_01}.png` plus `.game32.json` sidecars | Replace `PLACEHOLDER_sundered_keep_labyrinth_void_edge.png` where ocean/void collision boundaries need to read as cliffs, broken masonry, or collapsed keep edges instead of invisible walls. | Used around lower causeway ocean blockers and the Great Hall collapsed sea cut. |
...
| needed | Operator modular ranged-ready/fire suite | `custodian/content/sprites/operator/new_operator/modular/ranged/{upper_body_with_head,cape,weapon,fx}/operator__modular_{upper_body_with_head,wardrobe_cape,weapon,fx}__pistol__{pistol_ready,pistol_fire,pistol_recover,muzzle_flash,smoke_01}__{n,ne,e,se,s,sw,w,nw}__?f__96.png` plus runtime copies under `custodian/content/sprites/operator/runtime/modules/new_operator/` | Replace the fallback ranged stance/fire presentation with a true layered rig where upper body, cape, weapon, muzzle flash, and smoke act independently of lower-body idle/walk/run. | Ranged-ready input is wired, but production modular ranged clips do not exist yet; lower body intentionally remains locomotion-owned for V1. |
| needed | Operator sidearm ready/fire placeholder replacement suite | `custodian/content/sprites/operator/runtime/body/ranged/operator__body__ranged__sidearm_{ready,fire,recover,reload}_01__{n,ne,e,se,s,sw,w,nw}__?f__96.png` and matching weapon/FX layers under `custodian/content/sprites/operator/runtime/weapon/ranged/` and `custodian/content/sprites/operator/runtime/overlays/ranged/` | Replace the pistol sidearm fallback that currently reuses carbine/ranged placeholder animations when aim is held with no ranged primary equipped. | Runtime now has a separate sidearm slot and pistol profile; production art should keep lower-body locomotion independent and make one-hand sidearm aiming distinct from the 2h carbine. |
...
| needed | Operator split dodge/recovery directional body/FX suite | Full-body fallback: `custodian/content/sprites/operator/runtime/body/locomotion/operator__body__locomotion__{dodge,dodge_recovery}__{n,ne,e,se,s,sw,w,nw}__4f__96.png`; aim-back hop: `custodian/content/sprites/operator/runtime/body/locomotion/operator__body__locomotion__{dodge_backstep,dodge_backstep_recovery}__{n,ne,e,se,s,sw,w,nw}__4f__96.png`; optional modular/FX sources under `custodian/content/sprites/operator/new_operator/modular/dodge/`. | Replace the current one-direction dodge fallback with authored phase-split dodge, recovery, and aim-backstep coverage. | Runtime now supports separate dodge impulse and recovery timing. North legacy body/FX still works as fallback until these production strips are supplied. |
