...
- Full zone environment plates are `backdrop` families with `underlay` and `foreground` states. Their canvases are identical and register pixel-for-pixel. Zones 01–09 are now explicitly scene-bound at their locked envelope centers; the foreground remains visual occlusion only.
