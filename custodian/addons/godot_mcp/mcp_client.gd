...
const ENV_URL := "GODOT_MCP_URL"
const ENV_DISABLE_AUTOCONNECT := "GODOT_MCP_DISABLE_AUTOCONNECT"
...
func connect_to_server(url: String = DEFAULT_URL) -> void:
	if _is_autoconnect_disabled():
		print("[MCP] Autoconnect disabled via ", ENV_DISABLE_AUTOCONNECT)
		return
...
func _resolve_server_url(url: String) -> String:
	if url != DEFAULT_URL:
		return url
	var env_url: String = OS.get_environment(ENV_URL).strip_edges()
	if not env_url.is_empty():
		print("[MCP] Using URL from ", ENV_URL, ": ", env_url)
		return env_url
	return url


func _is_autoconnect_disabled() -> bool:
	var raw: String = OS.get_environment(ENV_DISABLE_AUTOCONNECT).to_lower().strip_edges()
