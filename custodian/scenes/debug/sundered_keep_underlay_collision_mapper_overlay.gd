...
func _collect_marker_points(state: Dictionary) -> Dictionary:
	...
	if underlay_scene != null and underlay_scene.has_method("get_underlay_authoring_marker_state"):
		var marker_state := underlay_scene.call("get_underlay_authoring_marker_state") as Dictionary
		for marker_id: String in marker_state.keys():
			var data := marker_state[marker_id] as Dictionary
			points[marker_id] = data.get("position", Vector2.ZERO) as Vector2
	var draft_markers := state.get("draft_markers", {}) as Dictionary
