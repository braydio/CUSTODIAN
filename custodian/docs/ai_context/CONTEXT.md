...
Twin Solaria is now canonically the Hub's detached **Crown Route Court**: a
Crown-class Continuity Adjudication Array that reconstructs damaged or incomplete
routes, tests reciprocal exposure, and requires Custodian judgment before
restoration. Its paired Solaria are continuity-reference towers, not literal suns,
and the surrounding Crown Verge remains intentionally unresolved rather than being
defined as ordinary outer space. See `design/05_levels/TWIN_SOLARIA.md`.
...
- Beginning scene: `res://scenes/awakening_first_return.tscn` is the application main scene. It implements Awakening / The First Return sections 01-10 as one continuous authored dungeon from the Crèche of Answerless Names to the Road of Witnesses South Reach, with `game/world/awakening/awakening_layout.gd` as the single spatial authority. The Field Terminal, Forum, Continuity Port, and first Contract belong to later sections and are not in this scene.
...
- Input prompts: `InputPromptService` is the presentation-only device-family and
  InputMap-label authority. Gameplay reads InputMap directly; UI consumes the
  service for hot-swapped keyboard/mouse versus Xbox text labels and must ignore
  trivial stick drift rather than turning the service into gameplay authority.
- Melee targeting is passive and aim-relative: `MeleeTargetResolver` owns deterministic scoring, hysteresis, shared reliable-reach math, and bounded pre-commit correction/drive calculation. Operator attack links capture that solution once; active facing and drive never home, `move_and_slide()` remains displacement authority, hitboxes remain damage authority, and parry-critical capture stays separate.
- Operator combat selection: Fists/unarmed is a first-class `OperatorWeaponDefinition` profile selected with `toggle_unarmed`; normal weapon cycling excludes Fists and only cycles armed profiles. The offhand secondary button (`aim_hold` / `attack_secondary`, right mouse or LT) contextually selects primary ranged-ready, equipped sidearm-ready, or tap parry / held guard. The semantic `heavy_attack` action (Shift/RB) owns melee/unarmed heavy intent, and `sprint` (Ctrl/L3) owns sprint. Two-handed raise/lower clips may retarget without restarting, use separate 0.22s/0.12s targets and a 0.70 ready threshold, and shots commit both the trigger-time world aim point and nearest authored visual sector. The 48x48 procedural reticle is the sole production ranged aim cue. Frame sockets own grip, muzzle/ejection position, layering, and debug calibration; at release, zero-spread direction runs from the current muzzle through the accepted aim point, with movement/heat/recoil spread afterward. Movement supports WASD/left stick, mouse/right-stick aim, and movement-first dodge with idle aiming backstep. Physical projectiles remain hit authority. Camera aim zoom/lead is camera-controller presentation state and must remain additive with shake/bounds rather than becoming Operator-owned final framing.
...
- Procgen void-underlay presentation uses one native-scale camera-following endless-forest stack for both generated-world and explicit-chasm inputs. Authoritative chasm cells additionally drive the absolute-z `-120` presentation-only `VoidCliffFace`: a deterministic 3–8-cell source-45 raised-rock fascia constrained to chasm, with no collision, navigation, surface, placement, or minimap authority. `RuntimeWalkableBoundary` remains physical authority; refined edge/corner/drop and contact-shadow art is deferred to Pass 2.
...
- The persistent procgen compound is now a deterministic 10–13-room semantic campus. `PersistentCompoundLayoutPlanner` derives seeded room reservations/topology without owning simulation; ProcGenTilemap remains physical and level-data authority, existing Sector nodes map by semantic `sector_id`, and shared MinimapView consumes rooms/connections. Anonymous `compound_buildings` and index-based Sector placement are legacy-data fallbacks only; authored `.tmj` interiors remain deferred.
...
- Awakening layer: `game/world/awakening/` owns the opening dungeon — spatial authority, progression orchestration, the Dust Lung transit lift, and the read-only console/plaque interactables — reusing the Operator, world camera, Black Reliquary HUD, the existing SidearmLocker, and the Road of Witnesses prototype as section 10. `game/world/home/` retains the reusable `FieldTerminalInteractable` and `SidearmLockerInteractable` for later sections.
...
5. **Awakening sections 11+.** The opening ten sections are implemented as a blockout and boot by default. The next content-flow milestone is the Hub/Forum stretch that introduces the Field Terminal, the Continuity Port, and the first Contract, and re-attaches contract prewarming and the handoff into the procgen sandbox at a point where a Contract has actually been surfaced. This requires the transition chain to handle first-run versus return.
