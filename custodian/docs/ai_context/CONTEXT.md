...
- Sundered Keep is the first production directed route. Route-owned ingress enters Vista and continues directly to the approved large Front Gate scene, with direct Front Gate-to-Vista backtracking and world exfil. The unfinished Return Causeway is quarantined behind `causeway_only` and must not be promoted without explicit user review. `RouteTraversalManager` is live under the world beside `LevelLoader`; it owns topology, profiles, sessions, rollback, exit binding, and state/cache behavior. Initial-entry post-commit rollback clears target loader authority synchronously, Front Gate session capture/restore includes siege objectives and the Great Hall ambush without replaying side effects, and profile validation rejects disconnected enabled topology. `LevelLoader` owns only instance staging and activation. Route-aware scaffold changes run full `RouteDefinition` validation against existing plus staged definitions before any write. Level-only registry ingress is wrapped in an internal one-node route. The future `WorldTransitionManager` remains limited to Compound/Campaign major-context changes. Runtime-persistent route state is not save-file persistence.
- Sundered Keep Camera 1 reveals the destination by removing occlusion inside one continuous storm/ocean world. The opaque storm horizon and ocean never crossfade; only the alpha-valid distant Keep landmark, a single reveal veil, and a restrained moonlight cue respond to the reversible entry envelope. The Keep remains visible after handback. Do not restore the retired baked moon/Keep/ocean wallpaper or activate Grand Vista, close fortress, foreground ruin, near-mist, or final-gate layers during Camera 1.
