...
- Sundered Keep is the first production directed route. Route-owned ingress enters Vista, production continues through Return Causeway to the real Front Gate scene, and explicit edges support backtracking and world exfil; debug profiles select direct-Keep or Causeway-only paths without scene booleans. The three nodes have distinct level IDs and named spawns. `RouteTraversalManager` is live under the world beside `LevelLoader`; it owns topology, profiles, sessions, rollback, exit binding, and state/cache behavior. `LevelLoader` owns only instance staging and activation. Level-only registry ingress is wrapped in an internal one-node route. The future `WorldTransitionManager` remains limited to Compound/Campaign major-context changes. Runtime-persistent route state is not save-file persistence.
