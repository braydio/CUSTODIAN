...
- Parry-critical ownership is split across a paired execution contract: the grunt owns enter/hold/recover vulnerability, atomic reservation, anchor/victim state, and exactly-once damage acceptance; the Operator owns contextual input, alignment, the shared 8-frame/12-FPS body/FX clock, frame-3 damage request, impact feedback, and unified cleanup. The authored pair is fixed south-facing and uses grunt `melee__` runtime asset names.
