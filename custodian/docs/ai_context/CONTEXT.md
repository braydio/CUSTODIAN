...
- Sundered Keep production access is `@world_origin -> vista_approach -> front_gate`. World ingress uses `playable_blackout`: procgen fades beneath the still-visible, controllable persistent Operator, and origin branches are isolated only after darkness. The continuous Approach and Outskirts scene contains transition arrival, First Vista, Return Parish, Near Vista, checkpoint, beach, and causeway foot. Its production mapper owns route-floor metadata, collision rails, semantic markers, subregions, and roof/occlusion records through three JSON documents. Beach spray/mist covers the `occluded_handoff` into the existing Front Gate `EntrySpawn`; Operator, camera, inventory, stamina, facing, and velocity remain on persistent runtime instances. Return Causeway stays `causeway_only`; the stage-cut route and complete procgen-frontage production rule are superseded.
