...
- Sundered Keep is the first registered authored destination under `game/world/sundered_keep/`. Production main-map placement is registry-driven through `WorldIngressSpawner`; its definition declares `EntrySpawn`, the `vista_approach` presentation profile, and explicit lifecycle policy, while the special procgen ingress → Vista Approach → optional Return Causeway → 112×80 front-gate chain remains temporarily approach-owned. Generic authored entry snapshots/restores `ProcGenRuntime`, `ConnectedMaps`, actor, camera, and UI state; world return clears `LevelLoader`, releases the outgoing node, and resets the ingress for re-entry. Named-spawn failure is authoritative and rolls that snapshot back. The planned `RouteTraversalManager` will own intra-campaign graph traversal; the future `WorldTransitionManager` remains limited to major context changes. The front-gate map is built from `content/levels/sundered_keep/sundered_keep_front_gate_large.json`; interaction state remains in `sundered_keep_map.gd`, including Return Mooring, key/gate, Great Hall, and minimap authority. `ContractWorldLoader`'s old Sundered placement helper remains compatibility/debug-only.
...
- Authored-level layer: `AuthoredLevel2D` owns production level content and requests loader-mediated, fail-closed return; generated playtest wrappers own temporary Operator/controller/camera nodes; registry definitions own ingress/spawn identity, presentation profile, and lifecycle policy; `WorldIngressSite` owns origin snapshots, structured restoration preflight, and ingress readiness; `LevelLoader` owns one active instance record and disables the outgoing node before origin restoration; and `WorldIngressSpawner` owns deterministic procgen placement. Failed restoration retains active loader/ingress ownership and reactivates the authored level. Route graphs/history are deliberately deferred to `RouteTraversalManager`. See `design/04_architecture/AUTHORED_LEVEL_AUTHORING_PIPELINE.md`.
