...
- Sundered Keep production access is `@world_origin -> front_gate -> @world_origin`, but the world-origin side now includes a complete intent-graph-owned procgen frontage before that route begins. `ASCENT_FIELD` owns its irregular route, terraces, cliffs, collision, minimap cells, dressing clearance, and generated terminal `gate_anchor`; authored Vista/Grand Vista assets own only semantic presentation and two reversible physical camera envelopes. No special-room rectangle, authored overlook pocket, route-master image, or mapper rail owns production ground. The former authored Vista remains behind `legacy_vista_debug`; Return Causeway remains behind `causeway_only`. `RouteTraversalManager` is live under the world beside `LevelLoader`; it owns topology, profiles, sessions, rollback, exit binding, and state/cache behavior only after the generated gate ingress is crossed. Initial-entry post-commit rollback clears target loader authority synchronously, Front Gate session capture/restore includes siege objectives and the Great Hall ambush without replaying side effects, and profile validation rejects disconnected enabled topology. `LevelLoader` owns only instance staging and activation. Route-aware scaffold changes run full `RouteDefinition` validation against existing plus staged definitions before any write. Level-only registry ingress is wrapped in an internal one-node route. The future `WorldTransitionManager` remains limited to Compound/Campaign major-context changes. Runtime-persistent route state is not save-file persistence.
- `SunderedKeepProcgenFrontagePresentation` reveals the destination while `ProcGenRuntime` remains visible and active. Two reversible physical camera envelopes consume normalized semantic anchors from the generated route, temporarily follow one presentation anchor, and compose storm/void, distant Keep, outer-wall, fortress, fog, and gate-shadow layers without owning floor or collision. Presentation persists downstream of each reveal while camera authority returns to gameplay. Do not restore baked route-master ground, fixed mapper rails, authored overlook pockets, or a separate scene lifecycle before the generated gate.
