...
- Parry-critical ownership is split across a paired execution contract: the grunt owns enter/hold/recover vulnerability, atomic reservation, anchor/victim state, and exactly-once damage acceptance; the Operator owns contextual input, shared-root alignment, the shared 8-frame/12-FPS body/FX clock, frame-3 damage request, impact feedback, and unified cleanup. The authored pair is fixed south-facing, uses grunt `melee__` runtime asset names, and requires a zero Operator offset because all full 96×96 exports preserve one canvas origin.
