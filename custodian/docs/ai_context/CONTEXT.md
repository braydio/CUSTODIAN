...
- Sundered Keep production access is a direct route: `@world_origin -> front_gate -> @world_origin`. The nearby Vista is a generated-world landmark sibling, not a route node. The former authored Vista is quarantined behind `legacy_vista_debug`; Return Causeway remains behind `causeway_only`. `RouteTraversalManager` is live under the world beside `LevelLoader`; it owns topology, profiles, sessions, rollback, exit binding, and state/cache behavior only after the real ingress is crossed. Initial-entry post-commit rollback clears target loader authority synchronously, Front Gate session capture/restore includes siege objectives and the Great Hall ambush without replaying side effects, and profile validation rejects disconnected enabled topology. `LevelLoader` owns only instance staging and activation. Route-aware scaffold changes run full `RouteDefinition` validation against existing plus staged definitions before any write. Level-only registry ingress is wrapped in an internal one-node route. The future `WorldTransitionManager` remains limited to Compound/Campaign major-context changes. Runtime-persistent route state is not save-file persistence.
- `SunderedKeepWorldVista` reveals the destination while `ProcGenRuntime` remains visible and active. Its one reversible physical camera envelope temporarily follows a presentation anchor, extends camera bounds, and removes atmospheric occlusion from the isolated Keep over the continuous storm horizon. It never starts a route, teleports the Operator, or owns collision. Do not restore the retired baked replacement wallpaper or activate Grand Vista, close fortress, foreground ruin, roof-fade, near-mist, or final-gate systems in this world landmark.
