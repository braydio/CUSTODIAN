...
- Sundered Keep production access is `@world_origin -> vista_approach -> front_gate`. Procgen owns a generated protected frontage and one reversible distant reveal, then a normal short fade enters the continuous authored Shore Parish / Outer Wall Approach. That mapper-owned scene contains the readable northbound route, one close checkpoint-detail composition, and a long east traverse; a second normal fade enters Front Gate. There is no production playable-blackout corridor, full-screen fog handoff, second procgen reveal, or authored Camera 2. Return Causeway stays `causeway_only`.
