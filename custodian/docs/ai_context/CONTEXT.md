...
- Sundered Keep production access is `@world_origin -> vista_approach -> front_gate`. Procgen owns ordinary generated terrain, a compact walkable north-edge ingress pocket, and registered ingress placement only. A normal short fade isolates the procgen world and enters the authored Vista Approach / Shore Parish, which exclusively owns ocean/storm and fortress presentation, route-master ground, camera reveal, boundary rails, approach enemies, and set dressing; a second normal fade enters Front Gate. The generated frontage and its vista stack are debug-only archived experiments. Return or failed entry restores procgen, Operator, camera-follow, and presentation-bounds state. Return Causeway stays `causeway_only`.
