...
- Completed: Added `use_field_patch` input on keyboard `B`; added Operator Field Patch tuning, carried count, timed commit, health restore, movement slow, status API, restock helper, damage/action/inventory-UI interruption, death/reset cleanup, and read-only compact HUD/debug status. Field Patch input is handled before build, so the existing `B` build behavior still works when a patch cannot start.
