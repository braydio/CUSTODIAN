...
- Constraints: Every guard entry goes through `block_enter`; critical-open phases keep independent actor roots after one parry knockback step; enemy owns opportunity/reservation/damage authority; Operator owns the shared eight-frame timeline after reservation; damage occurs once at frame 3; the authored south pair is not mirrored; its full-cell layers share a zero-offset root; grunt asset filenames use `melee__`.
- Acceptance: Enter advances to looping hold; expiry advances through recover; reservation is atomic; Operator and victim share the zero-offset execution root; semantic Operator body/FX and enemy victim clips share an 8-frame 12-FPS clock; frame-3 damage is exactly once; lethal/nonlethal/cancel cleanup is safe; focused Godot smokes pass.
- Completed: Added authored grunt enter/hold/recover registrations and phases; preserved the post-knockback enemy root and suppressed ordinary targeting through standalone recovery; realigned hold artwork 16 pixels right inside its uncropped cells for enter/hold/recover continuity; tokenized reservation plus begin/damage/finish/cancel callbacks; scene-owned zero-local execution anchor; fixed south shared-root alignment with no Operator separation offset; semantic paired playback; shared fixed-step frame clock with frame-3 one-shot damage; lethal, nonlethal, and interruption cleanup; DevConsole critical/execution grunt spawn presets; required-asset failures; focused smoke coverage; root-spec redirect and context drift cleanup.
