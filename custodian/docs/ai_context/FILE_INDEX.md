...
- `design/02_features/vehicles/AUTONOMOUS_COMBAT_DRONES.md` — complete V2 allied combat drone authority, including manager-owned fire discipline and close/far/free-roam follower commands; deferred logistics and presentation are tracked by the combat resource/readability umbrella
...
- `custodian/game/world/approaches/sundered_keep/sundered_keep_approach.tscn` and `.gd` — production Sundered Keep ingress approach scene with full ocean/sky/far-sea/distant-keep/fog/path/occlusion Sprite2D composition, visual-only `GrandVistaRoot` hero overlook layer, `GrandVistaGlueRoot` seam/contact/mask/spray overlays, soft feathering/grounding-shadow compositing, Entry/Reveal/SecondVista/Traverse/Return markers, perimeter `SegmentShape2D` rail collision, `VistaController`, and `ExitTransitionTrigger`
...
- `custodian/scenes/debug/sundered_keep_production_underlay_debug.tscn` — standalone visual review scene showing only `res://content/masters/sundered_keep/sundered_keep_main_overlay-production.png` fitted to the live `112x80` tile gameplay underlay rect; no gameplay map, tile, collision, or vista layers
...
- `custodian/game/actors/allies/combat_drone.gd` — fragile allied combat drone actor that follows/orbits the Custodian, supports FOLLOW/HOLD/INTERCEPT/RECALL modes, acquires local enemies, fires defense bullets, can be destroyed, exposes manager-controlled fire-at-will/follow-distance state, cancels queued bursts on hold fire, and enforces leashed local free roam
...
- `custodian/game/systems/drone/drone_manager.gd` — scene-mounted drone squad manager that spawns up to two animated allied droids near the operator, owns `drone_toggle_fire`/`drone_cycle_follow_distance` input, propagates squad state to current/future drones, and exposes squad mode/state APIs
- `custodian/game/systems/drone/drone_command_profile.gd` — drone tuning and mode constants for HP, speed, range, burst cadence, retreat threshold, tactical modes, follow-distance modes, spacing, and leash behavior
...
- `custodian/tools/validation/main_scene_allied_droid_smoke.gd` — focused smoke proving `game.tscn` routes `DroneManager` to the animated allied droid scene and manager-owned T/G command actions
- `custodian/tools/validation/drone_follower_commands_smoke.gd` — focused V2 smoke for squad state defaults/cycling, manager propagation, new-spawn state inheritance, and hold-fire burst cancellation
...
- `custodian/tools/validation/sundered_keep_approach_asset_audit.py` — validates Sundered Keep approach PNG dimensions plus required alpha on playable path exports and grand-vista parapet/ocean-spray overlays
