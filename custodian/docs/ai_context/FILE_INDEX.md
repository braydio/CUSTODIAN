...
- `custodian/tools/validation/grunt_parry_crit_reaction_smoke.gd` — focused headless smoke proving parried `enemy_grunt` instances play `stagger_s`, hold the last stagger frame as the critical-window placeholder, then route a follow-up hit to `crit_s` / `crit_fx_s` and `crit_recovery_s`
