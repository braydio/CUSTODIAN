...
- Build placement and full fabrication UI are still unimplemented. The first fab runtime spine and hand-placed harvestable test nodes are live; next slice should expose `ResourceLedger`, `FabPipeline`, and `BuildInventory` through terminal/debug UI or consume build tokens through placement.
