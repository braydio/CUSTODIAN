...
- Twin Solaria is canonically locked by `design/05_levels/TWIN_SOLARIA.md` as the Hub's detached **Crown Route Court**, a Crown-class Continuity Adjudication Array for reconstructing damaged/lost route solutions and reviewing reciprocal exposure. The paired Outbound/Reciprocal Solaria are continuity-reference towers, not literal suns; the surrounding Crown Verge remains intentionally unresolved rather than canonically ordinary outer space. The current `3500x3000` composite is preserved as a persistent documentation reference at `design/05_levels/reference/twin_solaria/twin_solaria_crown_route_court_reference.png`. Runtime remains unchanged: `res://scenes/twin_solaria_backdrop_test.tscn` is still a development-only fidelity preview using the development copy, live Operator/shared camera, image-matched bounds, and perimeter collision only; internal collision, route adjudication, and cross-map travel are not implemented.
