...
Mesh** is a separately typed `HAZARD_LAYER`, never an eighth polity. Pale Bell,
Indexer, Leaseholder, Choir, Buried-Kin, and Feral Defense concepts survive only
as the explicitly mapped traditions, schools, tendencies, local societies, and
historical automated-hazard source material described by
`design/03_world/factions/_FACTION_OVERVIEW.md`.

The narrative now treats post-Severing humanity as a second civilization built
around the material constraints of finite Lattice Domains. A CampaignRegion is
a transient runtime representation of operational access to a persistent
Domain. Mission resolution or scene teardown does not destroy that Domain;
Domains may expand, contract, change government, be abandoned, survive for long
periods, or eventually fail. Archive Engines stabilize damaged reality rather
than creating disposable worlds.

Faction gameplay implementation is **not complete**. No runtime behavior or
data changed in this migration. Runtime-shaped work is queued in
`design/02_features/factions/FACTION_IMPLEMENTATION_TRACKER.md`, beginning with
an audit and typed canonical definition/registry slice. The broader playable
idea bank lives in
...
The deterministic generator now publishes named structure sites for the Operations House, East Machine House, West Draft House, Alarm Gantry, and Yard Control Terminal. The East Machine House replaces the former authored-vault overlay with a 14×10 off-map interior, paired interaction doors, an open-center hardened-floor/machinery layout, collision boundaries, three renamed service-storage caches, an interior lighting zone, and one warm maintenance light.

Connected-map presentation now isolates the main procgen `ProcgenDepthBackdrop` during Carrow Yard and East Machine House travel, restoring it on return to the procgen world. This immediate leak fix does not change the separate underlay architecture or procgen terrain/navigation authority; biome-aware world-positioned depth scenery remains future work.

Carrow production art now enters through four Asset Pipeline V2 families: the District Transfer Frame, 64 Machine House floor states, 49 multi-cell wall modules, and 11 Machine House props. The travel gate is a layered Transfer Frame with discrete inactive/available/acquiring/active/failure presentation, while the East Machine House uses 140 registered floor sprites, an opaque oversized interior backing, and authored wall/prop placements over symbol-grid collision authority. Embedded service caches suppress the generic VaultStorage chest sprite while preserving storage behavior. Multi-cell walls remain sprites and are not forced into the 32 px TileSet registrar.
...
The fixed Ash-Bell Underground gameplay chapel remains `40x30` / `1280x960` at
the unchanged `(0,-1120)` origin. A separate `2594x1737` expanded arena
presentation master is registered there at native scale, with extended floor/perimeter art,
...
- The Ash-Bell lift now begins on an intentionally isolated cardinal-edge pocket selected deterministically across north/east/south/west candidates.
...
- Ash Bell and Sundered Keep edge presentation ownership is isolated. Ash Bell interleaves deterministic north/east/south/west pocket candidates, rotates only its map-facing approach composition, keeps lift/rider/shaft travel screen-upright, and transforms its `832x608` dressing clearance into an oriented world AABB. Its semantic isolated-overlook floor suppresses generic Floor TileMap visuals, and the inward-side pocket gap feeds the unchanged direction-independent Threadway runtime. The Sundered Keep procgen storm/ocean plate remains frontage-local and independently activated.
...
- Awakening lighting is now native and authored: `World/Lighting` owns one `CanvasModulate`, `DirectionalLight2D`, and `WorldLightingDirector`, with a neutral baseline for sections 01–04. Overlapping Dust Lung, south threshold, Undergate core, and north threshold zones create the first flashlight-dominant environment. Zone 06 is widened to `Rect2(-704, -4864, 1408, 1088)` and shaped as a narrow service throat, 1024px Mechanism Nave, west Register gallery, and north gate throat; its 1536×1216 underlay and true-alpha foreground are published and share the envelope center without replacing runtime lighting authority.
...
- High-resolution source art now has a deterministic pre-ingest converter at `custodian/tools/art/custodian_pixelart_converter.py`, exposed as `pixelart` after sourcing `tools/custodian_aliases.sh`. One command generates crisp nearest-neighbor, balanced box/palette, and bold clustered 96×96 candidates, presents a labeled comparison when a viewer is available, prompts for the selected output, warns on non-integer reduction ratios, and supports explicit non-interactive selection for repeatable automation. Converted PNGs remain cleanup masters and do not become runtime authority until reviewed and passed through the existing sprite intake pipeline.
...
- Melee presentation posture is live as a presentation-only READY/RELAXED axis driven by the existing `EngagementTracker`, with three-second draw grace and no new gameplay states. Generated E/W `idle_ready_01` and `idle_relaxed_01` body stacks remain under generic `melee_1h`; equipped authored-overlay weapons add matching posture animations from their own `OperatorWeaponDefinition.animation_profile` after the attack-overlay resource is installed. Vigil now supplies and runtime-selects both ready and relaxed four-frame weapon loops through `melee_1h_dagger`. Its first ready-state `fast_01` request plays the authored three-frame E/W lower/upper/weapon startup for 0.25 seconds before beginning the unchanged attack state; dodge-cancel and later combo links bypass this startup. Hidden legacy body animation may synchronize body-slaved overlays only while legacy attack/windup/recovery presentation visibly owns the body, and it cannot overwrite modular posture/run/draw weapon direction, frame, or frame progress. It is **not** a timing authority for a modular chain: visible frame cadence, hit-window scanning, frame ticks and completion all follow `_presentation_clock_sprite()`, and the deterministic gameplay length of an authored Fists link comes from `fast_chain_presentation_durations` rather than from any renderer. Vigil's six-frame E/W run is the first melee locomotion consumer of the existing Hybrid Weapon Socket System: the lower-body frame/progress clock drives wrapped smoothstep grip interpolation from generated upper-body socket records, the stopped normalized weapon overlay follows the hand with explicit `weapon_z`, and missing metadata restores the authored body-slaved strip without stale transforms. The authored south locomotion packages are now live without rotation: run uses the complete 12-frame lower/upper/Vigil weapon stack, walk uses the complete eight-frame stack, and incomplete directions retain fallback. The physically seven-cell incoming review source does not alter the canonical six-frame E/W body clock; cells 0–5 map to body frames 0–5 and cell 6 remains noncanonical pending an explicit body contract. Draw, posture, attacks, block/parry, and executions remain authored-strip presentation. Melee equip uses the canonical non-looping `draw_01` package; the obsolete lower-only `draw_weapon_01` family is retired. Gameplay action/lock states retain priority.
...
- Main contract-map Road Semantics V2 classifies intermittent ruined-Meridian fragments from the existing route/playability floor and derives at most one site-adjacent service apron. It adds no floor, repairs no connectivity, and changes no traversal, elevation, collision, or navigation. `soft_path` remains natural ground unless explicitly classified as a road; generic path metadata no longer means `ruined_road`. The apron also supplies the existing parking/staging export, foliage continues to avoid archived roads, ruined fragments, and soft paths, and the movement query retains 1.12 Operator / 1.35 vehicle multipliers on constructed road/hardstand surfaces (plus legacy soft-path behavior). The expensive wide-road carving/repair/parking/refresh/capture stack remains disabled behind `intent_main_roads_enabled = false`; its filled `_main_road_tiles` presentation and 15-piece role pack are debug-only. `procgen_road_surface_roles_smoke.gd` validates that archived opt-in implementation.
...
- A first authored hub-space prototype now exists at `res://scenes/hub_road_of_witnesses_prototype.tscn`. Its active presentation is five modular Road of Witnesses underlay/foreground plate pairs with authored collision kept in the Road controller; the historical monolithic PNG remains recoverable legacy art only. The north processional approach is deferred.
- Twin Solaria is canonically locked by `design/05_levels/TWIN_SOLARIA.md` and `design/03_world/lore/TWIN_SOLARIA_CROWN_INCIDENT.md` as the Hub's detached **Crown Route Court**. The namesake twins are giant aperture courts: surviving **Solarium I / Acquisition Aperture** in the upper-left and destroyed **Solarium II / Passage Aperture** in the missing upper-right Second Crown. Solarium II was deliberately amputated during a Severing-era Crown Reciprocal Incident after unacceptable reciprocal behavior was already underway; local amputation did not end the wider network crisis, while Ground Zero, final authorization, `NON-RECIPIENT`, and Ash-Bell identity remain unresolved. The current `3500x3000` composite is preserved as a persistent documentation reference at `design/05_levels/reference/twin_solaria/twin_solaria_crown_route_court_reference.png`. Runtime remains unchanged: `res://scenes/twin_solaria_backdrop_test.tscn` is still a development-only fidelity preview using the development copy, live Operator/shared camera, image-matched bounds, and perimeter collision only; internal collision, forensic interactions, route adjudication, Solarium I acquisition, and Crown-class passage are not implemented.
- The Ash-Bell / Forlorn-Ritualant encounter is a fixed authored Underground route, never a procgen special room. Its wrapper now owns a `112x128` / `3584x4096` cavern, translated `40x30` chapel at `(0,-1120)`, closed playable boundary loop, lower-lift arrival/dock (`(0,1440)` -> `(0,1696)`), up-screen player depth, camera bounds, E-only `return_world` exfil, and the black-covered route seam; the encounter site owns dialogue, pressure/tension/fountain state, thread anchors, combat and resolution. The surface ingress may occupy any cardinal world edge, but cardinal vectors position only semantic approach/blocker authority: existing cave, facade, shaft, and lift art always remains screen-upright. The current exterior facade is south-facing art reused as an explicit temporary fallback until north/east/west-facing presentation profiles exist. The retired procgen JSON remains absent. Reusable shader FX remain presentation-only. Normal game startup does not mount the dev spawner, and missing action/death art remains explicit in `REQUIRED_ASSETS.md` rather than silently invented.
- The Ritualant Underground now uses a 37-state Asset V2 environment family plus the dedicated two-state `ritualant_underground_hazard_fx` family. BALANCED remains the canonical landing apron. The authored presentation includes parallax depth/haze, polygon-clipped repeating ground, the native-scale `2594x1737` expanded arena plate, fixed rims/details/props, the revised chapel apron and presentation-only forelip, a completion-faded lower-quarter seal, arrival shaft layers, a one-shot distant chapel proxy, White Thread decals/warning/activation VFX, and priority camera zones that release to normal gameplay framing at the chapel. Hazard collision/tension, topology, route, lift, and encounter authority remain unchanged. Legacy site floor/perimeter visibility remains pending human mapper approval of the new master registration.
- Meridian civic authored levels now use the reviewed alpha-clean 512×512 wall atlas with a V2 196-entry/21-family semantic manifest covering the real 14×14 source grid with identity mapping into the runtime atlas. Rows/columns 14–15 are reserved and rotation/mirroring are disabled. Directional review is complete: 80 modules are exact-port auto-safe and 116 remain explicit/manual-only by design. A generated static GDScript catalog is runtime authority for topology/family lookup. The first Lower Quarter composition replaces three near-black masses with coherent floor-atlas roof fields, continuous wall caps, retaining faces, and six explicit arcade openings; collision/navigation remain unchanged. Physical civic props continue through the 224-entry native semantic manifest and exact placement authority; review-required compound IDs 177/201/212 still fail closed.
...
- Operator attacks now use `MeleeAttackProfile` resources for melee damage, range, arc, knockback, hit-stop, camera shake, cooldown, cancel timing, and phase movement; the old operator melee exports remain as deprecated fallbacks when a weapon profile is missing. Confirmed-contact feedback is owned by the contact that landed: `_apply_melee_hitbox_tick()` passes it to `_on_melee_hit_confirmed()`, which resolves hit-stop and camera amplitude once through authored contact override, then profile, then legacy fallback, and asks `Camera2D.on_attack_impact()` for that amplitude instead of a generic light/heavy value. An update that crosses two authored contacts therefore cannot give one contact's hit the other's feedback, and one contact is one camera impact and one hit-stop however many targets it swept.
...
- The operator scene now has a first layered modular rig for Fists movement/action separation: `ModularLowerBodySprite` and `ModularUpperBodySprite` both bind the one generated `content/sprites/operator/runtime/operator_runtime_frames.tres` as of C2a-R3; the former per-layer `operator_modular_{lower,upper}_body_frames.tres` compatibility resources stay on disk as C2b residue and no longer drive the rig. The lower body owns movement presentation (`idle_01`, `walk_01`, `run_01`) and resolves direction from movement; the upper body owns action/aim presentation and resolves direction from aim/action state, so the lower body can walk north while the upper body faces south. Fists idle prefers the modular lower/upper layer stack before the legacy authored-body stance fallback, so upper-body idle renders with lower-body idle. Slice C1 retired the hidden legacy animation clock: `LegacyFullBody` is hidden **and stopped** under modular presentation, and the frame value, frame tick, completion signal and overlay synchronization all follow `_presentation_clock_sprite()`, which only ever returns a visible layer. The sentence that used to stand here -- that the legacy body remained the hidden timing/source-of-truth sprite for attacks, hit windows, portal arrival, ranged states and reloads -- described the pre-C1 runtime and contradicted the visible-presentation-clock contract. The legacy body still owns those states when it is the *visible* owner; it no longer clocks them from behind a modular composition.
...
- Unarmed/offhand defense is owned by the actor-local `OperatorGuardController`, with typed tuning in `OperatorGuardConfig`. Guard is an explicit enter/hold/light-recoil/heavy-recoil/break/break-recovery/exit stance; parry retains windup/active/success/recovery phases within the same controller. The parry windup is 0.16667 s -- two frames of the authored 12 FPS `parry_01` strip -- so the 0.10 s active window lands on the frames where the guard is actually extended rather than during its anticipation; the animation is evidence for that value, never runtime authority. A confirmed parry success presents toward the accepted contact -- attacker position, then the negated hit direction, then the existing aim / visual-idle presentation fallback -- instead of re-reading aim first, so the success pose answers the blow it caught. There is no stored committed parry facing; the contract is that when a concrete contact direction exists, stale aim cannot override it. With no sidearm equipped, right mouse/LT enters guard-ready and reaches full protection after 0.10s; early guard provides partial mitigation. `HitStrength` differentiates posture cost and recoil, while attack-specific stamina overrides remain supported. Guard break collapses stamina to zero, creates a 0.40s vulnerable recovery, applies a 1.50s re-raise lockout, and requires a fresh press if guard remained held. Existing block-hitreact art is the temporary break fallback. A successful front-facing parry cancels the hit, preserves Falcon Reversal and ordinary counter behavior, refunds stamina, and opens the existing counter window.
...
- Fists/unarmed fast attack now presents a four-link modular E/W chain through canonical semantic selection: Fast 01–04 use 6/6/7/8-frame lower/upper clocks, authored contacts 4/4/4/5 (runtime 3/3/3/4), explicit authored E/W identities with no runtime flip, synchronized authored FX on every link, and non-looping terminal Fast 04 recovery. The first three links retain their prior non-timing balance; Fast 04 starts from Fast 03's balance with a **0.52-second** presentation target. The authored targets 0.30 / 0.32 / 0.37 / 0.52 own both the visible speed scale and the gameplay phase length, so the two cannot disagree. Older full-body variants remain preservation art outside this presentation path.
...
- Operator runtime decomposition is underway as a staged strangler migration, contracted in `design/04_architecture/OPERATOR_RUNTIME_ARCHITECTURE.md` and tracked in `custodian/docs/ai_context/task_packets/OPERATOR_RUNTIME_DECOMPOSITION.md`. The diagnosis is overlapping authority, not line count: `operator.gd` simultaneously owned input, locomotion, aim, combat, animation selection, rendering, loadout, interaction and debug concerns. `custodian/tools/validation/operator_architecture_debt_audit.py` freezes that as a per-file ledger; new or increased debt fails ordinary validation and `--final` requires zero. The ledger opened at 347 violations and stands at **97**. Slice A (contract, audit, characterization) is complete. **Slice B and B-final are complete**: body-renderer visibility authority now lives in `presentation/operator_body_presenter.gd`, with owner-scoped overlays, a transactional `present()` that validates before it mutates, and caller-side cancellation of a preempted rig's lifecycle. The `body_visibility_outside_presentation` zero was a false zero until B-final added `aliased_body_visibility_writes` (17 -> 0), which catches a body renderer bound to a loop variable or parameter. **Slice C1 is complete**: playback authority lives in `presentation/operator_animation_player.gd`, which owns HOW an already-resolved clip plays and nothing semantic; `animated_sprite_play_outside_presentation` went 94 -> 0. The hidden legacy animation clock is gone — `LegacyFullBody` is hidden AND stopped under modular presentation, and the frame value, frame tick, completion signal and overlay synchronization all follow `_presentation_clock_sprite()`, which only ever returns a visible layer. **Slice C2a is complete through R4/R4.1**: `OperatorAnimationSelector` is the semantic selection authority for the cut-over renderers, and `animated_sprite` is canonical. `AnimationResolver` (45 -> 18), `attack_fallback_animation` (16 -> 13) and `DirectionalAnimationFallback` (6 -> 4) are reduced but not yet zero; retiring the remainder is C2b, which is not started. An evidence pass first proved every legacy clip mapping against texture provenance, because the compatibility SpriteFrames are evidence about historical pixels, not semantic authority — two mappings inferred from clip names turned out to be wrong. `modular_sidearm_sprite` (C2a-R1) and `modular_upper_fx_sprite` (C2a-R2) are cut over and bound to `operator_runtime_frames.tres`; R2 also removed the last runtime `SpriteFrames` mutation (`_ensure_operator_critical_hitspark_animation()`). **C2a-R3 cut the body pair over atomically**: both modular body renderers now share the canonical SpriteFrames, the per-instance `duplicate(true)` fork and the secondary `operator_animation_catalog_frames.tres` copy are gone, and the cross-action mis-publications are corrected (walk NE/NW and run NE draw their own action, fast-windup north no longer draws recovery art). Transition entrypoints now acquire body ownership before showing a layer instead of relying on an earlier frame. Weapon socket tracks were rekeyed to the canonical upper-body identity — the runtime legacy-name translator is deleted — with one documented exception: `relaxed_01` has no authored socket calibration and explicitly shares the same sector's `stance_01` track. C2a-T1 and T1.1 first made authored FPS/loop/durations survive a cutover and froze that evidence. `animated_sprite` was the last renderer and is cut over as of R4/R4.1; the melee/weapon overlays remain, and the modular head and cape are retired from active composition rather than migrated. **Slice D is complete**: `operator/input/` is the sole owner of raw Operator input sampling -- `OperatorInputRouter` is the only `Input.*` reader, `OperatorInputFrame` is one fixed tick's intent with edges that fire on exactly the tick that observed them, and `OperatorAimController` owns the retained controller direction that `operator.gd` used to keep. The tick is now `_sample_input_frame` -> `_advance_simulation` (clocks, input handling, state changes) -> `_advance_movement` (movement intent, attack drive, the sole `move_and_slide()`), and `_process` is presentation only, so gameplay no longer depends on frame rate. A neutral controller stick retains its direction rather than falling through to a stale mouse position, and pointer movement rather than pointer presence is what hands aim back to the mouse -- `OperatorAimController` consumes `OperatorInputFrame.mouse_moved`, because `InputPromptService` switches to keyboard_mouse on any keyboard press, not just a mouse one. The Operator's override of `ControllableActor.process_input()` is real -- the base class stub is unchanged. An injected frame is adopted by the next fixed tick with `just_pressed`/`just_released` derived against the previous tick, so a replay, an AI or a vehicle can drive edge-triggered actions such as melee and the sidearm rather than only held-fire ranged. `input_calls_outside_input_dir` went 65 -> 0 and `gameplay_mutation_in_process` 12 -> 0, covered by `operator_input_frame` and `operator_fixed_tick_spine`, both of which own `operator/input/**`. The D.1 seal corrected three false render-tick audit exemptions: the ranged action timer gates firing, the animation state machine's state gates movement locks and weapon selection, and melee draw grace gates the Vigil ready-up bridge, so all three are on the fixed tick and only `_update_body_recoil` remains exempt. The D.2 seal closed the two authority seams that survived D.1. Externally supplied aim is now an explicit input-frame fact -- `OperatorInputFrame.external_control` plus a normalized `control_aim` -- that `OperatorAimController` ranks above every local source, conditional on neither `arrow_aim_enabled`, the `InputPromptService` device family, the retained stick nor the mouse latch; before D.2 the router filed the injected vector as `keyboard_aim`, which the aim authority reads only in arrow-aim mode (off by default), so `process_input()` documented and accepted a world-space aim direction and could silently ignore it, leaving external control able to press a button but not steer. And physical pointer motion is now event-derived: `InputPromptService` keeps `mouse_motion_generation`, a monotonic count advanced only by a qualifying `InputEventMouseMotion`, and the router compares that number instead of comparing `_get_world_mouse_position()` between ticks -- a camera-relative coordinate that `CameraController` moves on its own through follow smoothing, lookahead, ranged lead, threat framing, bob and shake, so a motionless mouse could otherwise take aim back from an active gamepad. `operator_input_aim_source` carries both negative controls against the real actor, including a live `Camera2D` at the path the Operator reads. The D.3 correction closed one behaviour D.2 changed by accident: the mouse-ownership latch started *set*, so a fresh keyboard/mouse session had no cursor aim until the player moved the mouse. The latch is now normally clear and means "blocked until motion because a protected source took aim", which only GAMEPAD and EXTERNAL set -- the reclaim rule without the birth rule.

**Slice C2b is partially complete (2026-09-24).** Three compatibility authorities are retired. `OperatorAnimationCatalog` is gone: comparing it against `operator_runtime_frames.tres` showed 675 identities against 582, with every one of the 179 it held additionally being a `legacy_*` entry and zero non-legacy identities unique to it, so it was residue rather than an authority; the generated JSON manifest stays as pipeline metadata. `attack_fallback_animation` went 13 -> 0, removed at the source -- the `MeleeAttackProfile` export, its value in ten attack profiles, the parameter and the write-only locals. Actor-local `SpriteFrames` construction went 5 -> 1 and runtime PNG loading is gone from `operator.gd`: the Knight test skin (documented stale since R4, and the last writer of `animated_sprite.sprite_frames`) is retired with its DevConsole command, the melee fast-chain FX builder is deleted as a zero-consumer path no weapon `fx_map` referenced, and dodge FX now binds the canonical runtime frames in the scene after verifying the swap is frame-, loop- and FPS-identical to what the builder produced from the same PNGs.

**Slice C2b.1 removed that blocker (2026-09-24).** The canonical
`operator_runtime_frames.tres` is now an immutable runtime database: equipping,
unequipping or cycling weapons changes which identity a renderer selects and
never what the database contains. `_install_weapon_body_frames()`,
`_install_melee_posture_weapon_frames()`, `_copy_runtime_animation()` and
`_apply_melee_weapon_animation_resources()` are gone, along with the per-weapon
`SpriteFrames` fields and the six resources they pointed at.
`operator_runtime_spine_immutable` snapshots the whole database -- names, frame
counts, fps, loop, every frame duration, and each frame's atlas source and region
-- cycles Vigil/unarmed/Cleaver/Vigil, and carries its own negative control.

The earlier diagnosis that per-weapon body and FX art was broadly missing was too
coarse. Measured against the database, most of it was already canonical:

- Vigil Fast 01/02 body and all three FX links are the shared `melee_1h` family,
  pixel-identical to what the compatibility resources played. They published at
  the 12 fps pipeline default because the body and FX layers had no timing
  sidecar, so C2b.1 authored those sidecars and republished at the authored
  18/14/13 -- the rates the *weapon* layer of the same actions already carried.
- The Vigil weapon layer needed nothing; it was already published weapon-scoped
  at 9 frames, 13 fps with the 1.5x final hold for Fast 03.
- Only Vigil's Fast 03 **body** needed new publication, because the dagger plays
  the first nine frames of the shared ten-frame strip. Those exact nine frames
  were copied region-for-region into a weapon-owned override
  (`weapon/vigil_pattern_dagger/melee_1h_dagger/attack/fast_03/{e,w}/full_body`),
  verified EXACT against the compatibility resource in pixels and timing.
- The Sword Cleaver never rendered `melee_1h_heavy` art at all: its resources
  pointed at the generic `melee_1h` fast_01 strip, the same one for all three
  links. It now selects the distinct authored `melee_1h_heavy` art per link at
...
- Operator modular production coverage is now inspectable with `custodian/tools/validation/operator_animation_contract_report.py`
  against `custodian/tools/validation/contracts/operator_animation_core.json`. General QA composites are produced by
...
- Lower Quarter authored-ground composition is live across Lower Quarter, West Gate Works, and Station IX. Every runtime paving source now comes from an exact region base, exact wear/damage coordinate, or exact line/marking range; no floor hashing or palette choice remains. Arrival, municipal roads, service roads, Market, Basin, Court, Wrong Street, West Gate rooms, and Station IX rooms therefore retain intentional material identities. Straight wall bands are temporarily pinned to top `(0,0)` and face `(1,4)` pending the dedicated topology/orientation wall audit. The V2 runtime
  ...
  review-only seam-neutralization pass keeps that exact allowlist, always
  bleeds the outermost pixel inward, and applies conservative dark-edge
...
`(6, -6626)`. The Road now owns five modular production plate pairs and separate
authored collision; `apply_camera_bounds` is off inside the Awakening, and a
`south_gate_gap_width` opens its southern boundary so the Approach joins it as
continuous walkable space. The north processional approach remains deferred. The
