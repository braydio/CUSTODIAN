...
- Operator runtime decomposition is underway as a staged strangler migration, contracted in `design/04_architecture/OPERATOR_RUNTIME_ARCHITECTURE.md` and tracked in `custodian/docs/ai_context/task_packets/OPERATOR_RUNTIME_DECOMPOSITION.md`. The diagnosis is overlapping authority, not line count: `operator.gd` is 540 KB and simultaneously owns input, locomotion, aim, combat, animation selection, rendering, loadout, interaction and debug concerns. `custodian/tools/validation/operator_architecture_debt_audit.py` freezes that as a per-file ledger of 347 violations across 12 ownership rules; new or increased debt fails ordinary validation and `--final` requires zero. Slice A (contract, audit, characterization, documentation drift) is complete. Slice B's one-body presentation invariant is live — `_set_body_presentation_owner()` is the single body authority and `operator_visual_ownership_smoke` asserts visible body owners either side of every handoff — but the authority still lives inside `operator.gd`, so extracting `presentation/operator_body_presenter.gd` remains open. Gameplay still advances on the render tick in 12 places; that moves to the fixed tick in Slice D.
