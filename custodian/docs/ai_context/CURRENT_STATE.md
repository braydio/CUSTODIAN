...
**C2b is not finished, and the remaining blocker is specific.** `animation_resolver` (18) and `directional_animation_fallback` (4) are unchanged because `_install_weapon_body_frames()` copies a weapon's `body_frames_resource` animations *into* the canonical `operator_runtime_frames.tres` object at runtime, so an equipped Vigil dagger adds `vigil_dagger_fast_03_right` to the canonical database in place. The compatibility tail of `_play_melee_anim_resolved()` is therefore live for armed melee, not archaeology -- an attempt to delete it as unreachable was caught by `operator_vigil_dagger`. Zeroing those two needs per-weapon body and FX art published as canonical identities (`weapon/<id>/<profile>/attack/...` exists for the weapon layer; the matching `fx` layer does not), which is Asset Pipeline V2 work rather than a call-site cleanup. The one remaining actor-local `SpriteFrames` is the per-weapon melee overlay install, blocked by the same missing identities. `operator.gd` measures 14,841 lines / 709 functions, down from 15,231 / 723. Slice E (`OperatorActionController`) and Slice F (domain extraction) remain.
