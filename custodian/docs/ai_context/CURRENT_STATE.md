...
- Shrumb cognitive pickups now render item-specific static sprites from `res://content/sprites/items/shrumb_drops/` with a lightweight procedural bob/pulse instead of the earlier colored placeholder square.
...
- Ruin prop definitions and production overlay/rubble PNGs still need to be authored under `custodian/content/props/ruins/data/prop_definitions/`, `extracted/`, and `overlays/`; the runtime system is ready but currently asset-definition empty.
