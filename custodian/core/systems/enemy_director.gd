...
func _resolve_dependencies() -> void:
	...
	if threat_model == null:
		threat_model = preload("res://core/systems/threat_model.gd").new()
	...
	if enemy_factory == null:
		enemy_factory = preload("res://core/systems/enemy_factory.gd").new()
...
func _rebuild_lanes() -> void:
	for child in get_children():
		if child.get_script() == preload("res://core/systems/assault_lane.gd"):
			child.queue_free()
	lanes.clear()
	for node in get_tree().get_nodes_in_group("enemy_spawn"):
		...
		if not lanes.has(key):
			var lane = preload("res://core/systems/assault_lane.gd").new()
			lane.lane_name = key
