...
Extracts the generated CUSTODIAN Black Reliquary UI component sheet into runtime
...
UI_SET = "ui_black_reliquary"
UI_FOLDER = "black_reliquary"
DISPLAY_NAME = "Black Reliquary UI Kit"
SCRIPT_PATH = "custodian/tools/art/extract_ui_black_reliquary_game32.py"
MASTER_SHEET_NAME = "ui_black_reliquary_components.png"
...
NINEPATCH_GROUP_OUTPUTS = {
    "dark_gold": {
        ...
        "notes": "Primary Black Reliquary charcoal panel with brass/gold trim.",
    },
    "deep": {
        ...
        "ui_role": "nine_patch_panel",
        "godot_node_hint": "NinePatchRect",
        "notes": "Deeper, less ornate Black Reliquary charcoal panel with brass trim.",
    },
}
...
def ui_root(project_root: Path) -> Path:
    return project_root / "custodian/content/ui" / UI_FOLDER


def project_res_path(project_root: Path, disk_path: Path) -> str:
    """
    Converts a disk path under custodian/ into a Godot res:// path.

    If --no-master-copy points to a source outside custodian/, return the absolute
    path string instead of crashing. Normal production runs copy into custodian/
    and therefore produce res:// paths.
    """
    custodian_root = (project_root / "custodian").resolve()
...
def assemble_ninepatch(
    ...
    col_w = [
        max(tl.width, l.width, bl.width),
        max(t.width, c.width, b.width),
        max(tr.width, r.width, br.width),
    ]
    row_h = [
        max(tl.height, t.height, tr.height),
        max(l.height, c.height, r.height),
        max(bl.height, b.height, br.height),
    ]
    ...
    out_dir = ui_root(project_root) / info["out_subdir"]
...
def domain_dir_for(project_root: Path, domain: str) -> Path:
    root = ui_root(project_root)

    if domain == "panel":
        return root / "panels"
    if domain == "panel_piece":
        return root / "panels/pieces"
    if domain == "divider":
        return root / "dividers"
    if domain == "ornament":
        return root / "ornaments"
    if domain == "icon":
        return root / "icons"
    if domain == "prompt":
        return root / "prompts"
    if domain in {"minimap", "minimap_marker"}:
        return root / "minimap"
...
def write_domain_manifests(
    ...
    for domain, assets in sorted(by_domain.items()):
        domain_dir = domain_dir_for(project_root, domain)
...
def write_top_level_manifest(project_root: Path, all_assets: list[dict[str, Any]], source_sheet: Path) -> None:
    out_path = ui_root(project_root) / "_manifest.game32.json"
    ...
    manifest = {
        ...
        "base_path": "res://content/ui/black_reliquary",
...
def write_doc_drift_review(project_root: Path) -> None:
    review_path = ui_root(project_root) / "_doc_drift_review.json"
    ...
    review = {
        ...
        "status": "needs_doc_update_if_accepted",
        "runtime_outputs": {
            "ui_assets": "custodian/content/ui/black_reliquary/",
        ...
        "note": "If the Black Reliquary UI kit is wired into active Godot HUD scenes, update active AI context docs.",
        "generated_at_utc": utc_now(),
    }
...
def main() -> None:
    ...
    master_sheet = project_root / "custodian/content/masters/ui" / MASTER_SHEET_NAME
    ...
    for spec in SPECS:
        ...
        out_dir = ui_root(project_root) / spec.out_subdir
        out_dir.mkdir(parents=True, exist_ok=True)
    ...
    print("  find custodian/content/ui/black_reliquary -type f | sort")
    print("  python - <<'PY'")
