...
func _validate_input_bindings() -> void:
	...
	_assert_true(not _action_has_joy_button("quick_item", JOY_BUTTON_DPAD_UP), "quick_item should not collide with Field Patch")
	_assert_true(_action_has_joy_button("use_field_patch", JOY_BUTTON_DPAD_UP), "Field Patch should include D-pad up")
	...
	_assert_true(not _action_has_joy_button("cycle_item_left", JOY_BUTTON_DPAD_LEFT), "cycle_item_left should not collide with weapon cycling")
	_assert_true(not _action_has_joy_button("cycle_item_right", JOY_BUTTON_DPAD_RIGHT), "cycle_item_right should not collide with weapon cycling")
	_assert_true(_action_has_joy_button("cycle_prev_weapon", JOY_BUTTON_DPAD_LEFT), "previous weapon should include D-pad left")
	_assert_true(_action_has_joy_button("cycle_next_weapon", JOY_BUTTON_DPAD_RIGHT), "next weapon should include D-pad right")
	_assert_true(_action_has_key("sprint", KEY_CTRL), "sprint should preserve Ctrl")
	_assert_true(_action_has_joy_button("sprint", JOY_BUTTON_LEFT_STICK), "sprint should include L3")
	_assert_true(_action_has_key("heavy_attack", KEY_SHIFT), "heavy attack should preserve Shift")
	_assert_true(_action_has_joy_button("heavy_attack", JOY_BUTTON_RIGHT_SHOULDER), "heavy attack should include RB")
	_assert_true(_action_has_key("sneak", KEY_CTRL), "sneak should move to Ctrl")
	_assert_true(_action_has_key("sneak", KEY_ALT), "sneak should provide a non-sprint keyboard binding")
	_assert_true(_action_has_joy_button("sneak", JOY_BUTTON_RIGHT_STICK), "sneak should include R3")
...
func _validate_offhand_input_trigger(root: Node, label: String, press: Callable, release: Callable) -> void:
	...
	press.call()
	operator.call("_sample_input_frame")
	operator.call("_advance_simulation", 0.016)
	_assert_true(operator.get("_parry_phase") == &"", "%s should not tap-parry from secondary alone" % label)
	_assert_true(operator.get("_block_phase") in [&"enter", &"hold"], "%s should enter guard through fixed-tick input routing" % label)
	...
	operator.call("_advance_simulation", float(operator.get("parry_min_guard_time_sec")) + 0.016)
	Input.action_press("fire_primary")
	operator.call("_sample_input_frame")
	operator.call("_advance_simulation", 0.016)
	_assert_true(operator.get("_parry_phase") == &"windup", "%s + primary should start parry windup through fixed-tick input routing" % label)
...
func _validate_dodge_fx_overlay(operator: Node) -> void:
	...
	_assert_true(body_sprite.sprite_frames.get_frame_count(&"shared/transition/dodge_01/s/full_body") == 9, "south dodge should use all 9 authored frames")
