...
func _init() -> void:
	...
	assert(pursuit_frames.has_animation(&"walk_n"))
	assert(pursuit_frames.has_animation(&"relaxed_walk_n"))
	assert(pursuit_frames.has_animation(&"walk_s"))
	assert(pursuit_frames.has_animation(&"relaxed_walk_s"))
	assert(pursuit_frames.get_frame_count(&"walk_n") == 8)
	assert(pursuit_frames.get_frame_count(&"relaxed_walk_n") == 8)
	assert(pursuit_frames.get_frame_count(&"walk_s") == 8)
	assert(pursuit_frames.get_frame_count(&"relaxed_walk_s") == 8)
	assert(PURSUIT_SET.get_animation_name(PURSUIT_SET.resolve_clip(&"locomotion.walk", &"n"), &"body") == &"walk_n")
	assert(PURSUIT_SET.get_animation_name(PURSUIT_SET.resolve_clip(&"locomotion.relaxed_walk", &"n"), &"body") == &"relaxed_walk_n")
	assert(PURSUIT_SET.get_animation_name(PURSUIT_SET.resolve_clip(&"locomotion.walk", &"s"), &"body") == &"walk_s")
	assert(PURSUIT_SET.get_animation_name(PURSUIT_SET.resolve_clip(&"locomotion.relaxed_walk", &"s"), &"body") == &"relaxed_walk_s")
