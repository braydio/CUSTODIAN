...
func _init() -> void:
	...
	a.set("_local_exposure",0.12); _require(float(a.get_presentation_state().precipitation_alpha) <= 0.12,"indoor precipitation was not suppressed")
	...
	print("world_environment_smoke: PASS weather=%s" % a.debug_get_state().weather_current); map.free(); a.free(); b.free(); lighting.free(); quit(0)

func _require(condition: bool, message: String) -> void:
	if condition: return
	push_error("world_environment_smoke: " + message); quit(1)
