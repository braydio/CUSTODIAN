...
func _run() -> void:
	...
	tilemap.set_seed(271828)
	assert(tilemap.get_generation_seed() == 271828)
	var navigation_root := map.get_node("NavigationRegion2D")
	var structural_children := navigation_root.get_children()
	var chasm_cells_before := (tilemap.get("_chasm_cells") as Dictionary).duplicate(true)
	assert(tilemap.get_underlay_profile_override() == "ENDLESS_FOREST")
	var map_backdrop := map.get_node("DepthBackdrop") as ProcgenDepthBackdrop
	tilemap.call("_refresh_depth_backdrop")
	...
	assert(navigation_root.get_children() == structural_children)
	assert((tilemap.get("_chasm_cells") as Dictionary) == chasm_cells_before)
	assert(map_backdrop.find_children("*", "TileMapLayer", true, false).is_empty())
	...
	tilemap.set_underlay_profile_override("ENDLESS_FOREST")
	assert(map_backdrop.get_underlay_profile_id() == &"endless_forest")
	assert(navigation_root.get_children() == structural_children)
	assert((tilemap.get("_chasm_cells") as Dictionary) == chasm_cells_before)
	map.queue_free()
