...
class PreviewControls(Static):
    ...
    def show(self, *, frame: int, frames: int, fps: float, playing: bool, loop: bool, source: str, zoom: str = "auto") -> None:
        state = "PLAY" if playing else "PAUSE"
        self.update(f"{state}   FRAME {frame + 1} / {frames}   REVIEW FPS {fps:.1f}   LOOP {'✓' if loop else '×'}   SOURCE: {source.upper()}   ZOOM: {zoom.upper()}")
