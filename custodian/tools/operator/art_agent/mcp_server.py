...
_TOOL_SPECS: dict[str, dict[str, tuple[dict[str, Any], bool]]] = {
    ...
    "operator_art_source_handoff": {
        "session": (_STRING, True), "destination_name": (_STRING, True),
        "replace": (_BOOLEAN, False), "dry_run": (_BOOLEAN, False),
    },
...
_DESCRIPTIONS: dict[str, str] = {
    ...
    "operator_art_source_handoff": "Stage a reviewed candidate in the specialized Operator sprite inbox. Use explicit replace plus dry_run for a same-semantic replacement report; this does not publish canonical/runtime art.",
...
class OperatorArtMCP:
    ...
    def tools(self) -> dict[str, Callable[..., Any]]:
        ...
        return {
            ...
            "operator_art_source_handoff": lambda session,destination_name,replace=False,dry_run=False,**_:source.handoff(Path(session),destination_name=destination_name,replace=replace,dry_run=dry_run),
