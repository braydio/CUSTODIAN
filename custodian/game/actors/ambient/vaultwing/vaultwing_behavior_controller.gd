...
func request_interest(position: Vector2, source: Node2D = null) -> void:
	if state == State.DEAD or state in [State.DIVE_STRIKE, State.CLIMB_OUT, State.RETREAT]: return
	target_position = position; target_position_valid = true; target = source; _interest_elapsed = 0.0; _enter(State.CIRCLE_INTEREST)

func request_dive(position: Vector2, source: Node2D = null) -> void:
	if state == State.DEAD or actor == null: return
	target_position = position; target_position_valid = true; target = source; _enter(State.DIVE_WINDUP)
...
func notify_damage(amount: float) -> void:
	...
	if actor != null and actor.health / maxf(actor.max_health, 1.0) <= profile.low_health_ratio:
		retreat_vector = (actor.global_position - home_position).normalized()
		if retreat_vector.length_squared() < 0.01: retreat_vector = -strike_vector
		_enter(State.RETREAT)
	elif band == Band.ATTACK and amount >= profile.air_stagger_threshold: _enter(State.AIR_STAGGER)
	elif band == Band.GROUND and state not in [State.GROUND_STAGGER, State.DEAD]: _enter(State.GROUND_STAGGER)
	if not _has_valid_target():
		var player := get_tree().get_first_node_in_group("player") as Node2D
		if player != null: request_interest(player.global_position, player)
...
func step(delta: float) -> void:
	...
	match state:
		State.HIGH_PATROL:
			band = Band.HIGH; actor.velocity = patrol_direction * profile.patrol_speed
			if state_elapsed >= 2.0:
				if _should_use_perch() and _select_perch(): _enter(State.LAND)
				else: patrol_direction = patrol_direction.rotated(_rng.randf_range(-0.55, 0.55)).normalized(); _enter(State.HIGH_PATROL)
		State.CIRCLE_INTEREST:
			band = Band.HIGH; actor.velocity = patrol_direction * profile.patrol_speed * 0.55; _interest_elapsed += delta
		...
		State.DIVE_STRIKE:
			band = Band.ATTACK; actor.velocity = strike_vector * profile.dive_speed
			if not strike_contact_tested and state_elapsed >= 0.08 and state_elapsed <= 0.22:
				strike_contact_tested = true
				if _hit_target_with_shape(profile.dive_damage, &"dash"): strike_hit = true; _log_event(&"vaultwing_dive_hit", {"target": target.name if is_instance_valid(target) else ""})
			if state_elapsed >= profile.dive_strike_seconds:
				if not strike_hit: _log_event(&"vaultwing_dive_missed", {})
				_enter(State.CLIMB_OUT)
		State.CLIMB_OUT:
			band = Band.ATTACK; actor.velocity = (-strike_vector + Vector2.UP * 0.8).normalized() * profile.climb_speed
			if state_elapsed >= profile.climb_out_seconds: _enter(State.HIGH_PATROL)
		State.PERCH_IDLE, State.PERCH_ALERT:
			band = Band.PERCHED
			if state == State.PERCH_IDLE and state_elapsed >= profile.perch_dwell_seconds: _enter(State.TAKEOFF)
		State.LAND:
			band = Band.GROUND
			if selected_perch_valid:
				var to_perch := selected_perch - actor.global_position
				if to_perch.length() > profile.perch_approach_radius: actor.velocity = to_perch.normalized() * profile.ground_speed
				else: selected_perch_valid = false
			if state_elapsed >= profile.land_seconds and not selected_perch_valid: _enter(State.PERCH_IDLE if not perch_positions.is_empty() else State.GROUND_IDLE)
		State.GROUND_IDLE:
			band = Band.GROUND
		...
		State.GROUND_ATTACK:
			band = Band.GROUND
			if not strike_contact_tested and state_elapsed >= 0.12 and state_elapsed <= 0.28:
				strike_contact_tested = true; strike_hit = _hit_target_with_shape(profile.bite_damage, &"melee")
			if state_elapsed >= 0.52: _enter(State.GROUND_STALK if _has_valid_target() else State.GROUND_IDLE)
		State.TAKEOFF:
			band = Band.GROUND if state_elapsed < profile.takeoff_seconds * 0.35 else Band.HIGH
			if state_elapsed >= profile.takeoff_seconds: _enter(State.HIGH_PATROL)
		State.AIR_STAGGER:
			band = Band.ATTACK
		...
		State.RETREAT:
			band = Band.ATTACK if state_elapsed < 0.25 else Band.HIGH; actor.velocity = retreat_vector * profile.climb_speed
...
func _enter(next_state: State) -> void:
	...
	match state:
		State.HIGH_PATROL, State.CIRCLE_INTEREST: band = Band.HIGH
...
func _target_is_in_strike_shape() -> bool:
	...
	var direction: Vector2 = strike_vector if state == State.DIVE_STRIKE else actor.facing_direction
