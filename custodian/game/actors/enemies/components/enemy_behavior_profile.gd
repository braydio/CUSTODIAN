...
static func create_profile(id: StringName) -> Resource:
	...
	match id:
		...
		&"pursuit_frame":
			profile.profile_id = &"pursuit_frame"
			profile.display_name = "Pursuit Frame"
			profile.aggression_weight = 0.95
			profile.theft_weight = 0.0
			profile.sabotage_weight = 0.05
			profile.self_preservation_weight = 0.10
			profile.curiosity_weight = 0.35
			profile.vision_range_px = 300.0
			profile.vision_cone_degrees = 118.0
			profile.peripheral_vision_mult = 0.70
			profile.hearing_range_px = 210.0
			profile.investigation_memory_sec = 7.0
			profile.lost_sight_memory_sec = 4.5
			profile.detection_gain_per_sec = 2.15
			profile.detection_decay_per_sec = 0.45
			profile.detection_notice_threshold = 0.22
			profile.detection_alert_threshold = 0.80
			profile.operator_awareness_bubble_px = 225.0
			profile.operator_awareness_score = 215.0
			profile.can_steal_resources = false
			profile.can_sabotage_storage = false
			profile.morale_max = 115.0
			profile.morale_panic_threshold = 8.0
			profile.morale_loss_on_ally_death = 7.0
			profile.morale_loss_on_stagger = 10.0
			profile.morale_loss_on_turret_hit = 6.0
			profile.patrol_speed = 64.0
			profile.investigate_speed = 88.0
			profile.engage_speed = 112.0
			profile.flee_speed = 100.0
			profile.objective_speed = 90.0
			profile.ambient_activity_weight = 0.40
			profile.ambient_activity_duration_sec = 2.5
			profile.noncombat_warning_seconds = 0.18
		_:
