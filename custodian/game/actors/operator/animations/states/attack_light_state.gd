...
func _init(state_name: String = "attack_light"):
	name = state_name
	can_interrupt = true
...
func enter() -> void:
	...
	if state_machine and state_machine.actor and state_machine.actor.has_method("start_attack"):
		state_machine.actor.call("start_attack", "melee_light")
...
func trigger_damage_frame() -> void:
	...
	state_machine.trigger_event("damage_frame", "melee_light")


func update(delta: float) -> String:
	if state_machine and state_machine.actor and state_machine.actor.has_method("is_attack_state_complete"):
		if bool(state_machine.actor.call("is_attack_state_complete", "light")):
			return "idle"
	return name
