...
func enter() -> void:
	...
	if state_machine and state_machine.actor and state_machine.actor.has_method("start_attack"):
		...
		if state_machine.can_play_animation(&"melee_2h_fast_right"):
			state_machine.play_animation(&"melee_2h_fast_right")
		elif state_machine.can_play_animation(&"melee_2h_fast"):
			state_machine.play_animation(&"melee_2h_fast")
...
func update(delta: float) -> String:
	if state_machine and state_machine.actor and state_machine.actor.has_method("is_attack_state_complete"):
		if bool(state_machine.actor.call("is_attack_state_complete", "fast")):
			return "idle"
	return name
