...
@export var activation_texture: Texture2D = preload("res://content/sprites/misc/terminal_activation_sequence.png")
@export var animation_frame_size: Vector2i = Vector2i(64, 64)
@export_range(2, 16, 1) var animation_frame_count: int = 5
...
func _ensure_terminal_sprite() -> void:
	if _terminal_sprite != null:
		return
	...
	_build_terminal_frames()


func _build_terminal_frames() -> void:
	...
	var max_frames: int = maxi(1, texture_width / frame_width)
