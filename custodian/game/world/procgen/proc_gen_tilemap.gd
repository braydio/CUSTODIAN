...
var _sundered_keep_shoreline_plan: Dictionary = {}
...
const ROAD_PIECE_MANIFEST_PATH := "res://content/tiles/roads_paths/runtime/roads/surface/road_surface_piece_manifest.game32.json"
const ROAD_PIECE_EXPORT_ROOT := "res://content/tiles/roads_paths/runtime/roads/surface"
...
var _road_piece_defs_by_surface_role: Dictionary = {}
...
func _load_road_piece_manifest() -> void:
	...
	_road_piece_defs_by_surface_role = _load_road_surface_role_manifest(
		road_piece_manifest_path,
		ROAD_PIECE_EXPORT_ROOT
	)
	_path_piece_defs_by_mask = _load_surface_piece_manifest(path_piece_manifest_path, PATH_PIECE_EXPORT_ROOT, true)


func _load_road_surface_role_manifest(manifest_path: String, export_root: String) -> Dictionary:
	...
	for piece_variant in pieces as Array:
		...
		var surface_role := String(piece.get("surface_role", "")).strip_edges().to_lower()
		var file_path := String(piece.get("file", ""))
		if surface_role.is_empty() or file_path.is_empty():
			continue
		...
		var defs: Array = defs_by_role.get(surface_role, [])
		defs.append(stored)
		defs_by_role[surface_role] = defs
	return defs_by_role
...
func _spawn_road_piece_decals() -> void:
	if not road_piece_decals_enabled \
			or (_road_piece_defs_by_mask.is_empty() and _road_piece_defs_by_surface_role.is_empty()):
		return
...
func _reveal_road_surface_piece_decal(tile: Vector2i) -> void:
	if not road_piece_decals_enabled or _road_piece_defs_by_surface_role.is_empty():
		return
	if _road_piece_parent == null or not is_instance_valid(_road_piece_parent):
		_road_piece_parent = _find_or_create_road_piece_parent()
...
func _select_road_surface_piece_definition(tile: Vector2i, surface_role: String) -> Dictionary:
	var candidates: Array = _road_piece_defs_by_surface_role.get(surface_role, [])
	if candidates.is_empty() and surface_role != "center":
		candidates = _road_piece_defs_by_surface_role.get("center", [])
	if candidates.is_empty():
		return {}
	var index := _tile_noise_hash(tile + Vector2i(719, 1471)) % candidates.size()
...
func _spawn_road_piece_decal(tile: Vector2i, piece: Dictionary, surface_kind: String = "road") -> void:
	...
	sprite.set_meta("surface_kind", surface_kind)
	sprite.set_meta("surface_tile", tile)
	if piece.has("surface_role"):
		sprite.set_meta("surface_role", String(piece.get("surface_role", "")))
...
func _apply_sundered_keep_frontage_floor_visuals() -> void:
	...
	_sundered_keep_shoreline_plan = _build_sundered_keep_shoreline_plan()
	for entry_variant in _sundered_keep_shoreline_plan.get("floor_band", []):
		var entry := entry_variant as Dictionary
		var cell := entry.get("cell", Vector2i.ZERO) as Vector2i
		var source_id := int(entry.get("source_id", -1))
		if source_id == 129:
			_apply_terrain_tile_visual(cell, "sundered_keep_cliff_rock_floor_01")
		elif source_id == 130:
			_apply_terrain_tile_visual(cell, "sundered_keep_cliff_rock_floor_cracked_01")
		elif source_id == 131:
			_apply_terrain_tile_visual(cell, "sundered_keep_main_courtyard_flagstone_wet_01")
		visual_candidates[cell] = true
	var source_counts := {129: 0, 130: 0, 131: 0, 132: 0}
	for cell_variant in visual_candidates.keys():
		var source_id := floor_tilemap.get_cell_source_id(cell_variant as Vector2i)
...
func _build_sundered_keep_shoreline_plan() -> Dictionary:
	if _sundered_keep_frontage.is_empty():
		return {}
	var seed_value := 0
	if procgen_node != null and "seed" in procgen_node:
		seed_value = int(procgen_node.seed)
	var cell_world_size := 32.0
	if floor_tilemap != null:
		cell_world_size = floor_tilemap.to_global(
			floor_tilemap.map_to_local(Vector2i.RIGHT)
		).distance_to(floor_tilemap.to_global(
			floor_tilemap.map_to_local(Vector2i.ZERO)
		))
	return SUNDERED_KEEP_SHORELINE_COMPOSITOR.build_plan(
...
func _rebuild_nonwalkable_surface_visuals() -> void:
	...
	if _sundered_keep_shore_overlay_parent == null:
		...
		nonwalkable_surface_overlay_tilemap.add_child(
			_sundered_keep_shore_overlay_parent
		)
	SUNDERED_KEEP_SHORELINE_COMPOSITOR.apply_foam(
		_sundered_keep_shoreline_plan,
		nonwalkable_surface_overlay_tilemap,
		_sundered_keep_shore_overlay_parent
	)
...
func _rebuild_sundered_keep_coastline_presentation() -> void:
	...
	var presentation_scale := _sundered_keep_coastline_parent.global_transform.get_scale()
	var result: Dictionary = SUNDERED_KEEP_SHORELINE_COMPOSITOR.build_cliff_presentation(
		_sundered_keep_shoreline_plan,
		_sundered_keep_coastline_parent,
		local_cell_size,
		presentation_scale
	...
	_sundered_keep_coastline_parent.set_meta(
		"macro_cliff_count",
		int(result.get("cliff_count", 0))
	)
	_sundered_keep_coastline_parent.set_meta(
		"shoreline_plan_fingerprint",
		SUNDERED_KEEP_SHORELINE_COMPOSITOR.plan_fingerprint(
			_sundered_keep_shoreline_plan
		)
	)
...
func debug_get_sundered_keep_shoreline_plan() -> Dictionary:
	return _sundered_keep_shoreline_plan.duplicate(true)


func debug_save_sundered_keep_shoreline_fixture(output_path: String) -> Error:
	if _sundered_keep_frontage.is_empty() or _ocean_cells.is_empty():
	...
	file.store_string(SUNDERED_KEEP_SHORELINE_COMPOSITOR.fixture_to_json(
		seed_value,
		_generated_floor_cells,
		_ocean_cells,
		{
			"frontage_floor_rect": _cell_dictionary_bounds(
				_sundered_keep_frontage.get("floor_cells", {}) as Dictionary
			),
...
func debug_get_road_piece_decal_surface_role_counts() -> Dictionary:
	var counts: Dictionary = {}
	for node in _road_piece_nodes:
		if not node.has_meta("surface_role"):
			continue
