...
const OVERVIEW_PREVIEW_SIZE := 448
...
func get_preview_size() -> int:
	return OVERVIEW_PREVIEW_SIZE if _overview_mode else PREVIEW_SIZE
...
func local_to_world(local_pos: Vector2, control_size: Vector2) -> Vector2:
	...
	var image_size := Vector2(
		float(render_bounds.get("image_width", get_preview_size())),
		float(render_bounds.get("image_height", get_preview_size()))
	)
