...
const DEFAULT_VISTA_HORIZON_PATH := "res://content/masters/sundered_keep/sundered_keep_hilltop_vista.png"
const DEFAULT_VISTA_FOG_PATH := ""
...
@export_group("Vista Reveal")
@export var vista_reveal_enabled := true
@export_file("*.png") var vista_horizon_path: String = DEFAULT_VISTA_HORIZON_PATH
@export_file("*.png") var vista_fog_path: String = DEFAULT_VISTA_FOG_PATH
@export var vista_reveal_start_tile: Vector2i = Vector2i(56, 68)
@export var vista_reveal_full_tile: Vector2i = Vector2i(56, 52)
@export var vista_world_rect_tiles: Rect2i = Rect2i(Vector2i(0, -18), Vector2i(112, 34))
@export_range(0.0, 1.0, 0.01) var vista_max_alpha := 1.0
@export_range(0.1, 20.0, 0.1) var vista_smoothing_speed := 4.5
@export var vista_camera_blend_enabled := true
@export var vista_camera_shift := Vector2(0.0, -96.0)
@export_range(0.75, 1.25, 0.01) var vista_zoom_scale := 0.94
...
var _vista_root: Node2D = null
var _vista_horizon: CanvasItem = null
var _vista_fog_band: Polygon2D = null
var _vista_progress := 0.0
var _camera_baseline_captured := false
var _camera_baseline_offset := Vector2.ZERO
var _camera_baseline_zoom := Vector2.ONE
var _stats := {
	"floors": 0,
	"edges": 0,
	"walls": 0,
...
func get_sundered_keep_debug_state() -> Dictionary:
	return {
		...
		"vista_reveal_enabled": vista_reveal_enabled,
		"vista_progress": _vista_progress,
		"vista_has_horizon": _vista_horizon != null and is_instance_valid(_vista_horizon),
		"vista_start_tile": vista_reveal_start_tile,
		"vista_full_tile": vista_reveal_full_tile,
	}
...
func _build_from_level_data(data: Dictionary) -> void:
	...
	_build_ocean_backdrop()
	_build_vista_reveal_layers(data.get("vista_reveal", {}))
...
func _build_vista_reveal_layers(config: Dictionary = {}) -> void:
	var layer := _layers.get("VistaReveal", null) as Node2D
	if layer == null:
		layer = Node2D.new()
		layer.name = "VistaReveal"
		layer.z_as_relative = false
		layer.z_index = -140
		add_child(layer)
		_layers["VistaReveal"] = layer
	...
	_vista_root = layer
	_vista_horizon = null
	_vista_fog_band = null
	_vista_progress = 0.0
	_vista_root.visible = vista_reveal_enabled
	_vista_root.modulate.a = 0.0

	if not vista_reveal_enabled:
		return

	var rect_tiles := _array_to_rect2i(config.get("rect_tiles", [
		vista_world_rect_tiles.position.x,
		vista_world_rect_tiles.position.y,
		vista_world_rect_tiles.size.x,
		vista_world_rect_tiles.size.y,
	]))
	if rect_tiles.size.x <= 0 or rect_tiles.size.y <= 0:
		rect_tiles = vista_world_rect_tiles

	var horizon_texture_path := str(config.get("horizon_texture_path", vista_horizon_path))
	var horizon_texture: Texture2D = null
	if not horizon_texture_path.is_empty() and ResourceLoader.exists(horizon_texture_path):
		horizon_texture = _load_texture(horizon_texture_path)

	if horizon_texture != null:
		var horizon := Sprite2D.new()
		horizon.name = "VistaHorizonMatte"
		horizon.centered = false
		horizon.texture = horizon_texture
		horizon.texture_filter = CanvasItem.TEXTURE_FILTER_LINEAR
		horizon.position = _tile_top_left(rect_tiles.position)
		horizon.scale = Vector2(
			(float(rect_tiles.size.x) * TILE_SIZE) / max(1.0, float(horizon_texture.get_width())),
			(float(rect_tiles.size.y) * TILE_SIZE) / max(1.0, float(horizon_texture.get_height()))
		)
		layer.add_child(horizon)
		_vista_horizon = horizon
	else:
		_vista_horizon = _add_vista_fallback_horizon(layer, rect_tiles)

	var fog_texture_path := str(config.get("fog_texture_path", vista_fog_path))
	var fog_texture: Texture2D = null
	if not fog_texture_path.is_empty() and ResourceLoader.exists(fog_texture_path):
		fog_texture = _load_texture(fog_texture_path)

	if fog_texture != null:
		var fog_sprite := Sprite2D.new()
		fog_sprite.name = "VistaFogBand"
		fog_sprite.centered = false
		fog_sprite.texture = fog_texture
		fog_sprite.texture_filter = CanvasItem.TEXTURE_FILTER_LINEAR
		fog_sprite.position = _tile_top_left(Vector2i(rect_tiles.position.x, rect_tiles.position.y + int(rect_tiles.size.y * 0.58)))
		fog_sprite.scale = Vector2(
			(float(rect_tiles.size.x) * TILE_SIZE) / max(1.0, float(fog_texture.get_width())),
			(float(rect_tiles.size.y) * TILE_SIZE * 0.42) / max(1.0, float(fog_texture.get_height()))
		)
		layer.add_child(fog_sprite)
	else:
		var fog_height := maxi(4, int(round(float(rect_tiles.size.y) * 0.42)))
		var fog_y := rect_tiles.position.y + rect_tiles.size.y - fog_height
		_vista_fog_band = _add_rect_overlay(
			layer,
			"VistaFogBand",
			Rect2i(Vector2i(rect_tiles.position.x, fog_y), Vector2i(rect_tiles.size.x, fog_height)),
			Color(0.12, 0.15, 0.17, 0.0)
		)


func _add_vista_fallback_horizon(layer: Node2D, rect_tiles: Rect2i) -> Polygon2D:
	var horizon := _add_rect_overlay(
		layer,
		"VistaFallbackHorizon",
		rect_tiles,
		Color(0.016, 0.024, 0.036, 0.88)
	)

	var far_sea := _add_rect_overlay(
		layer,
		"VistaFallbackFarSea",
		Rect2i(
			Vector2i(rect_tiles.position.x, rect_tiles.position.y + int(rect_tiles.size.y * 0.45)),
			Vector2i(rect_tiles.size.x, maxi(1, int(rect_tiles.size.y * 0.55)))
		),
		Color(0.012, 0.038, 0.055, 0.82)
	)
	far_sea.z_index = 1
...
func _update_vista_reveal(delta: float) -> void:
	if not vista_reveal_enabled or _vista_root == null or not is_instance_valid(_vista_root):
		return
	...
	var target_progress := _compute_vista_reveal_progress(actor.global_position)
	var smoothing := 1.0 - exp(-vista_smoothing_speed * delta)
	_vista_progress = lerpf(_vista_progress, target_progress, smoothing)

	var eased := smoothstep(0.0, 1.0, _vista_progress)
	_apply_vista_reveal(eased)


func _compute_vista_reveal_progress(actor_global_position: Vector2) -> float:
	var start_pos := _tile_center(vista_reveal_start_tile)
	var full_pos := _tile_center(vista_reveal_full_tile)
	var actor_pos := to_local(actor_global_position)
	var path := full_pos - start_pos
	var path_len_sq := path.length_squared()
	if path_len_sq <= 0.001:
		return 0.0
	return clamp((actor_pos - start_pos).dot(path) / path_len_sq, 0.0, 1.0)


func _apply_vista_reveal(progress: float) -> void:
	_vista_root.modulate.a = lerpf(0.0, vista_max_alpha, progress)

	if _vista_horizon != null and is_instance_valid(_vista_horizon):
		_vista_horizon.modulate.a = smoothstep(0.18, 1.0, progress)

	if _vista_fog_band != null and is_instance_valid(_vista_fog_band):
		var fog_color := _vista_fog_band.color
		fog_color.a = lerpf(0.0, 0.62, smoothstep(0.0, 0.75, progress))
		_vista_fog_band.color = fog_color

	if vista_camera_blend_enabled:
		_apply_vista_camera_blend(progress)


func _apply_vista_camera_blend(progress: float) -> void:
	var camera := _runtime_camera()
	if camera == null:
		return

	if not _camera_baseline_captured:
		_camera_baseline_offset = camera.offset
		_camera_baseline_zoom = camera.zoom
		_camera_baseline_captured = true

	camera.offset = _camera_baseline_offset.lerp(_camera_baseline_offset + vista_camera_shift, progress)
	camera.zoom = _camera_baseline_zoom.lerp(_camera_baseline_zoom * vista_zoom_scale, progress)


func _runtime_camera() -> Camera2D:
...
func _reset_vista_reveal() -> void:
	_vista_progress = 0.0
	if _vista_root != null and is_instance_valid(_vista_root):
		_vista_root.modulate.a = 0.0
	if _vista_horizon != null and is_instance_valid(_vista_horizon):
		_vista_horizon.modulate.a = 0.0
	if _vista_fog_band != null and is_instance_valid(_vista_fog_band):
		var fog_color := _vista_fog_band.color
		fog_color.a = 0.0
		_vista_fog_band.color = fog_color
