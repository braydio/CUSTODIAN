...
def apply_tutorial(state: GameState, result):
    ...
    if state.is_failed:
        _append_lines(result, ["[TUTORIAL] SESSION FAILED. QUICKSTART ABORTED."])
    ...
    if step.key == "REPAIR_CHECK" and not _has_damaged_structures(state):
        _append_lines(result, ["[TUTORIAL] NO DAMAGED STRUCTURES DETECTED."])
        _finish_tutorial(state, result)
        return result
    ...
    if step.condition(state, last, result):
        ...
        if next_prompt:
            _append_lines(result, next_prompt)
        else:
            _finish_tutorial(state, result)
        return result
    ...
    if state.tutorial_last_hint_step != state.tutorial_step:
        ...
        _append_lines(result, hint)
        state.tutorial_last_hint_step = state.tutorial_step

    return result


def _finish_tutorial(state: GameState, result) -> None:
    _append_lines(
        result,
        [
            "-----",
...
def _append_lines(result, lines: list[str]) -> None:
    if not lines:
        return
    if result.lines:
