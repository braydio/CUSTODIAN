...
| partial | Operator unarmed parry body and FX suite | `custodian/content/sprites/operator/new_operator/modular/parry/operator__modular_{lower_body,upper_body,upper_fx}__unarmed__parry_{start,success,recovery,fx}_01__{n,ne,e,se,s,sw,w,nw}__?f__96.png` plus generated runtime modules | Complete authored primary-from-guard parry, perfect-parry success, parry recovery, and impact FX coverage. | Baseline `parry_01` is live for guard-triggered parry/success at 12 FPS with **east, north and west** coverage across all three layers: lower body, upper body and FX (`unarmed/defense/parry_01/{e,n,w}/{lower_body,upper_body,fx}`). East and north are 5 frames; west is 6, the extra frame being a presentation settle that does not alter gameplay timing. Successful parry currently uses clearly labeled `PLACEHOLDER_unarmed_parry_success_fx*` animations sourced from the normal parry FX strip until a dedicated success burst lands. Still needed: dedicated recovery, stronger success/impact variants, **south and diagonal** body/FX coverage, and final audio. South and the diagonals are genuinely absent -- the runtime projects them onto the authored e/n/w sectors rather than faking art. |
