...
| Status | Asset | Target Path | Purpose | Notes |
|---|---|---|---|---|
| partial | Forlorn-Ritualant NPC runtime sprites | `custodian/content/sprites/enemies/enemy_forlorn_ritualant/runtime/body/` | Complete readable locomotion, reaction, and encounter-defining action animation on the live 128×128 contract. | The scene now uses `AnimatedSprite2D` with south-facing 128×128 idle, hostile-idle, four-frame rise, pin-strike, and thread-pull sheets. Still needed: directional eight-frame walk/drag cycles, turning, hit/stagger reactions, an eight-frame rise matching the design timing, `ninth_answer`, and `orra_late`; the older 48×64/ColorRect note was stale. |
