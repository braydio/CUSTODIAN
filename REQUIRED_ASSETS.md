...
| Status | Asset | Target Path | Purpose | Notes |
|---|---|---|---|---|
| partial | P0/P1 zone plates — nine `awakening_*_environment` families | `custodian/content/levels/awakening/<zone>/<owner>_{underlay,foreground}_<WxH>.png` | Replace each section's blockout presentation without changing spatial authority. | Zones 01–05 now have underlay + foreground ingested and bound; Undergate (06) and Late Service (09) are explicitly wired in the live scene. Gate Plaza (07) and Custodian Approach (08) remain published per their contracts but are not yet explicitly bound in `awakening_first_return.tscn`. |
