...
| Status | Asset | Target Path | Purpose | Notes |
|---|---|---|---|---|
| partial | Forlorn-Ritualant NPC runtime sprites | `custodian/content/sprites/enemies/forlorn_ritualant/runtime/body/` | Complete readable locomotion and reaction animation on the live 128×128 contract. | Idle, hostile-idle, four-frame rise, Pin Strike, Thread Pull, eight-frame Ninth Answer, and eight-frame Orra Comes Late are live. Still needed: directional walk/drag, turning, reactions, and the eight-frame rise. |
| partial | Unarrived Saint apparition sprite | `custodian/content/tiles/encounters/ritualant_set/unnarrived_apparration_01.png` | Show the delayed behind-player apparition when thread snaps, Orra Comes Late resolves, or the site is defiled. | A real Sprite2D-backed apparition is live; final production polish/directional apparition variants remain open. |
...
| partial | Dry Fountain apparition states | `custodian/content/tiles/encounters/ritualant_set/runtime/{dry_fountain_ghost_frames,black_water_fountain_frames}.tres` plus `fountain_cracked_anchored_ring_01.png` | Support ABSENT/GHOST/BLACK_WATER/CRACKED_ANCHORED fountain states. | Ghost and black-water states are AnimatedSprite2D-backed and the anchored ring is textured; final state-transition polish remains open. |
| needed | Ash-Bell audio cues | `custodian/content/audio/sfx/ash_bell/thread_strain_*.wav`, `custodian/content/audio/sfx/ash_bell/reverse_chime_*.wav`, `custodian/content/audio/ambience/ash_bell/ash_bell_room_bed_*.ogg`, `custodian/content/audio/sfx/ash_bell/silence_pulse_*.wav`, `custodian/content/audio/sfx/ash_bell/stilling_pin_drag_*.wav` | Provide thread strain, reversed chime, room bed, silence-pulse, and pin-drag feedback for the encounter mechanics. | Runtime has state hooks but no authored audio node hierarchy, review-ready assets, or mixer mute implementation yet. |
...
| Status | Asset | Target Path | Purpose | Notes |
|---|---|---|---|---|
| needed | Production procgen road/path decal pack | `custodian/content/tiles/roads_paths/runtime/roads/{standard,gothic}/pieces/*.png`, `custodian/content/tiles/roads_paths/runtime/paths/pieces/*.png`, and matching manifests | Replace temporary road/path decal art when those visual packs receive production review. | Production Road Semantics V2 uses intermittent semantic road fragments and service hardstand over existing floor. The old wide-road decal renderer is disabled/debug-only; its 32px role vocabulary is `center`, `edge`, `outer corner`, and `inner corner` (not lane offsets). `soft_path` remains on the separate connection-bitmask path manifest and is not synonymous with ruined roadway. |
...
| Status | Asset | Target Path | Purpose | Notes |
|---|---|---|---|---|
| partial | Enemy Savage rushdown animation suite | `custodian/content/sprites/enemies/enemy_savage/runtime/body/{locomotion,melee}/enemy_savage__body__*__{n,ne,e,se,s,sw,w,nw}__?f__96.png`, optional FX under `runtime/fx/` | Complete readable movement, two-hit pressure, pounce commitment, reactions, and death. | E/W eight-frame run strips are wired; missing directions use nearest-sector presentation fallback. E/W pounce body/FX strips exist but remain unwired. Still needed: N/S/diagonal movement coverage plus chain, reaction, and death art/playback. |
...
| Status | Asset | Target Path | Purpose | Notes |
|---|---|---|---|---|
| needed | Light-body authored impact variants 02 and 03 | `custodian/content/audio/sfx/combat/hit_light_body_{02,03}.wav` | Complete the requested balanced/sharp/dull three-take light-hit family before enabling bounded runtime pitch variation. | `hit_light_body_01.wav` is supplied and live. Export 02/03 as 48 kHz, 24-bit, mono PCM WAV with normalize/dither/reverb off; keep perceived loudness within ±1 dB. Runtime pitch randomization remains disabled until both land. |
...
| partial | Operator unarmed parry body and FX suite | Authored through **Asset Pipeline V2**; canonical source `custodian/content/sprites/operator/source/animations/unarmed/defense/parry_01/` and `.../parry_success_01/`, generated runtime `custodian/content/sprites/operator/runtime/animations/unarmed/defense/...`. The old `new_operator/modular/parry/...` authoring tree this row used to name no longer exists. | Complete authored primary-from-guard parry, perfect-parry success, parry recovery, and impact FX coverage. | Baseline `parry_01` is live for guard-triggered parry/success at 12 FPS with **east, north and west** coverage across all three layers: lower body, upper body and FX (`unarmed/defense/parry_01/{e,n,w}/{lower_body,upper_body,fx}`). East and north are 5 frames; west is 6, the extra frame being a presentation settle that does not alter gameplay timing. Parry success presentation is **live, not placeholder**: a dedicated world-space burst (`game/vfx/combat/parry_success_burst_vfx.tscn`), a contact spark, and positional success SFX, plus a dedicated 5-frame `parry_success_01` FX strip in east and west. What is still missing is separate modular **body-layer** success and recovery art -- `parry_success_01` publishes FX only, so the success pose reuses the `parry_01` body strip. Still needed: modular body-layer success and recovery art, stronger impact variants, and **south and diagonal** body/FX coverage. Audio is not a gap here -- the positional parry-success sound is live; the only missing beat is a dedicated *miss* cue, tracked in its own row below. South and the diagonals are genuinely absent -- the runtime projects those requests onto the authored e/n/w sectors rather than faking art. |
...
| partial | Operator charge-windup and clean-link directional coverage | `custodian/content/sprites/operator/runtime/actions/{dodge_charge,dodge_chain}/body/` | Complete the dedicated ratio-held charge and repeatable clean-link presentation families. | E/S/W five-frame charge and four-frame link strips are live. Missing N/diagonal coverage uses deterministic nearest-sector fallback; hard pivots retain the full-dodge atlas. |
| partial | Operator modular idle-hit reaction coverage | `custodian/content/sprites/operator/runtime/modules/new_operator/{lower_body,upper_body,head}/actions/{unarmed,hooded}/idle_hitreact_01/` | Complete synchronized modular light-recoil direction coverage. | N/S lower/upper and optional hooded-head layers are live. Diagonals resolve to N/S and E/W ties remain stable; dedicated E/W/diagonal art is still absent. |
