...
| partial | Operator unarmed parry body and FX suite | Authored through **Asset Pipeline V2**; canonical source `custodian/content/sprites/operator/source/animations/unarmed/defense/parry_01/` and `.../parry_success_01/`, generated runtime `custodian/content/sprites/operator/runtime/animations/unarmed/defense/...`. The old `new_operator/modular/parry/...` authoring tree this row used to name no longer exists. | Complete authored primary-from-guard parry, perfect-parry success, parry recovery, and impact FX coverage. | Baseline `parry_01` is live for guard-triggered parry/success at 12 FPS with **east, north and west** coverage across all three layers: lower body, upper body and FX (`unarmed/defense/parry_01/{e,n,w}/{lower_body,upper_body,fx}`). East and north are 5 frames; west is 6, the extra frame being a presentation settle that does not alter gameplay timing. Parry success presentation is **live, not placeholder**: a dedicated world-space burst (`game/vfx/combat/parry_success_burst_vfx.tscn`), a contact spark, and positional success SFX, plus a dedicated 5-frame `parry_success_01` FX strip in east and west. What is still missing is separate modular **body-layer** success and recovery art -- `parry_success_01` publishes FX only, so the success pose reuses the `parry_01` body strip. Still needed: modular body-layer success and recovery art, stronger impact variants, **south and diagonal** body/FX coverage, and final audio. South and the diagonals are genuinely absent -- the runtime projects those requests onto the authored e/n/w sectors rather than faking art. |
